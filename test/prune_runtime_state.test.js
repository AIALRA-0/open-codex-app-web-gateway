...
test("runtime prune script dry-runs then applies bounded artifact cleanup", () => {
  ...
  try {
    ...
    const uploadsDir = path.join(stateDir, "local-uploads", "uploads");
    ...
    const oldUpload = writeDirectoryWithMtime(path.join(uploadsDir, "upload_old"), old);
    const freshUpload = writeDirectoryWithMtime(path.join(uploadsDir, "upload_fresh"), fresh);

    const dryRun = runPrune(root, ["--max-age-hours", "1", "--max-files", "1"]);
    assert.equal(dryRun.status, 0, dryRun.stderr);
    const dryRunReport = JSON.parse(dryRun.stdout);
    assert.equal(dryRunReport.dry_run, true);
    assert.equal(dryRunReport.summary.selected, 4);
    ...
    assert.equal(fs.existsSync(oldUpload), true);
    ...
    assert.equal(appliedReport.summary.deleted, 4);
    ...
    assert.equal(fs.existsSync(freshUpload), true);
...
function writeDirectoryWithMtime(dirPath, mtimeMs) {
  fs.mkdirSync(dirPath, { recursive: true });
  writeFileWithMtime(path.join(dirPath, "upload.json"), "{}", mtimeMs);
  const date = new Date(mtimeMs);
