...
test("POST /v1/responses validates seed before provider calls", async () => {
  await withMockProvider(async (_req, res, call) => {
    assert.equal(call.body.seed, 42);
    res.writeHead(200, { "content-type": "application/json" });
    res.end(JSON.stringify({
      id: "chatcmpl_responses_seed_boundary",
      object: "chat.completion",
      created: 100,
      model: "mock-model",
      choices: [{
        index: 0,
        message: { role: "assistant", content: "responses seed ok" },
        finish_reason: "stop",
      }],
      usage: { prompt_tokens: 2, completion_tokens: 2, total_tokens: 4 },
    }));
    ...
    const baseUrl = `http://127.0.0.1:${bridgeAddress.port}`;
    const invalidCases = [
      { seed: "42", message: "seed must be an integer" },
      { seed: 42.5, message: "seed must be an integer" },
      { seed: [], message: "seed must be an integer" },
      { seed: {}, message: "seed must be an integer" },
      {
        seed: 1e21,
        message: "seed must be an integer between -9223372036854776000 and 9223372036854776000",
      },
    ];
    for (const invalidCase of invalidCases) {
      const response = await fetch(`${baseUrl}/v1/responses`, {
        ...
        body: JSON.stringify({
          model: "mock-model",
          input: "Check seed validation.",
          seed: invalidCase.seed,
        }),
      });
      assert.equal(response.status, 400);
      assert.deepEqual(await response.json(), {
    ...
    const valid = await fetch(`${baseUrl}/v1/responses`, {
      ...
      body: JSON.stringify({
        model: "mock-model",
        input: "Check seed boundary.",
        seed: 42,
      }),
    });
    assert.equal(valid.status, 200);
    assert.equal((await valid.json()).output[0].content[0].text, "responses seed ok");
    assert.equal(requests.length, 1);
  });
});
...
test("POST /v1/responses validates top_logprobs range before provider calls", async () => {
  await withMockProvider(async (_req, res, call) => {
    ...
    assert.equal(call.body.top_logprobs, 0);
    res.writeHead(200, { "content-type": "application/json" });
    res.end(JSON.stringify({
      id: "chatcmpl_top_logprobs_boundary",
      object: "chat.completion",
      created: 100,
      model: "mock-model",
      choices: [{
    ...
    const baseUrl = `http://127.0.0.1:${bridgeAddress.port}`;
    const invalidValues = [-1, 21, 1.5, "2"];
    for (const value of invalidValues) {
      const response = await fetch(`${baseUrl}/v1/responses`, {
        ...
        body: JSON.stringify({
          model: "mock-model",
          input: "Check top logprobs validation.",
          top_logprobs: value,
        }),
      });
      assert.equal(response.status, 400);
    ...
    const valid = await fetch(`${baseUrl}/v1/responses`, {
      ...
      body: JSON.stringify({
        model: "mock-model",
        input: "Check top logprobs lower boundary.",
        top_logprobs: 0,
      }),
    });
    assert.equal(valid.status, 200);
    assert.equal((await valid.json()).output[0].content[0].text, "boundary ok");
    assert.equal(requests.length, 1);
  });
});
...
test("POST /v1/responses validates sampling parameter ranges before provider calls", async () => {
  await withMockProvider(async (_req, res, call) => {
    ...
    res.end(JSON.stringify({
      ...
      choices: [{
        index: 0,
        message: { role: "assistant", content: "sampling ok" },
        finish_reason: "stop",
      }],
      usage: { prompt_tokens: 3, completion_tokens: 2, total_tokens: 5 },
    }));
    ...
    for (const invalidCase of invalidCases) {
      const response = await fetch(`${baseUrl}/v1/responses`, {
        ...
        body: JSON.stringify({
          model: "mock-model",
          input: "Check sampling validation.",
          ...invalidCase.body,
        }),
      });
      assert.equal(response.status, 400);
    ...
    assert.equal(valid.status, 200);
    assert.equal((await valid.json()).output[0].content[0].text, "sampling ok");
    assert.equal(requests.length, 1);
  });
});

test("POST /v1/responses validates stop sequences before provider calls", async () => {
  await withMockProvider(async (_req, res, call) => {
    assert.deepEqual(call.body.stop, ["<a>", "<b>", "<c>", "<d>"]);
    res.writeHead(200, { "content-type": "application/json" });
    res.end(JSON.stringify({
      id: "chatcmpl_stop_boundary",
      object: "chat.completion",
      created: 100,
      model: "mock-model",
      choices: [{
        index: 0,
        message: { role: "assistant", content: "stop ok" },
        finish_reason: "stop",
      }],
    }));
  }, async ({ bridgeAddress, requests }) => {
    ...
    const valid = await fetch(`${baseUrl}/v1/responses`, {
      ...
      body: JSON.stringify({
        model: "mock-model",
        input: "Check stop boundary.",
        stop: ["<a>", "<b>", "<c>", "<d>"],
      }),
    });
    assert.equal(valid.status, 200);
    assert.equal((await valid.json()).output[0].content[0].text, "stop ok");
    assert.equal(requests.length, 1);
  });
});
...
test("POST /v1/responses validates Chat-native n before provider calls", async () => {
  await withMockProvider(async (_req, res, call) => {
    ...
    res.writeHead(200, { "content-type": "application/json" });
    res.end(JSON.stringify({
      id: "chatcmpl_response_n_boundary",
      object: "chat.completion",
      created: 100,
      model: "mock-model",
      choices: [{
        index: 0,
        message: { role: "assistant", content: "response n ok" },
        finish_reason: "stop",
      }],
    }));
  }, async ({ bridgeAddress, requests }) => {
    const baseUrl = `http://127.0.0.1:${bridgeAddress.port}`;
    const invalidValues = [0, 129, 1.5, "2", [], {}];
    for (const value of invalidValues) {
      const response = await fetch(`${baseUrl}/v1/responses`, {
        method: "POST",
        headers: { "content-type": "application/json" },
      ...
      assert.deepEqual(await response.json(), {
        error: {
          message: "n must be an integer between 1 and 128",
          type: "invalid_request_error",
          param: "n",
          code: "invalid_request_parameter",
        },
    ...
    const valid = await fetch(`${baseUrl}/v1/responses`, {
      ...
      body: JSON.stringify({
        model: "mock-model",
        input: "Check n boundary.",
        n: 128,
      }),
    });
    assert.equal(valid.status, 200);
    assert.equal((await valid.json()).output[0].content[0].text, "response n ok");
    assert.equal(requests.length, 1);
  });
});
...
test("POST /v1/responses validates logit_bias before provider calls", async () => {
  await withMockProvider(async (_req, res, call) => {
    ...
    res.writeHead(200, { "content-type": "application/json" });
    res.end(JSON.stringify({
      id: "chatcmpl_logit_bias_boundary",
      object: "chat.completion",
      created: 100,
      model: "mock-model",
      choices: [{
        index: 0,
        message: { role: "assistant", content: "logit bias ok" },
        finish_reason: "stop",
      }],
    }));
  }, async ({ bridgeAddress, requests }) => {
    ...
    for (const invalidCase of invalidCases) {
      const response = await fetch(`${baseUrl}/v1/responses`, {
        ...
        body: JSON.stringify({
          model: "mock-model",
          input: "Check logit_bias validation.",
          ...invalidCase.body,
        }),
      });
      assert.equal(response.status, 400);
    ...
    const valid = await fetch(`${baseUrl}/v1/responses`, {
      ...
      body: JSON.stringify({
        model: "mock-model",
        input: "Check logit_bias boundary.",
        logit_bias: { "7": -100, "8": 100 },
      }),
    });
    assert.equal(valid.status, 200);
    assert.equal((await valid.json()).output[0].content[0].text, "logit bias ok");
    assert.equal(requests.length, 1);
  });
});
...
test("POST /v1/responses maps input_image detail to Chat image content parts", async () => {
  await withMockProvider(async (_req, res, call) => {
    assert.deepEqual(call.body.messages, [{
      role: "user",
      content: [
    ...
    res.writeHead(200, { "content-type": "application/json" });
    res.end(JSON.stringify({
      id: "chatcmpl_image_detail",
      object: "chat.completion",
      created: 100,
      model: "mock-model",
      choices: [{
        index: 0,
        message: { role: "assistant", content: "image detail ok" },
        finish_reason: "stop",
      }],
...
test("POST /v1/responses loads deferred remote MCP tools through hosted tool_search", async () => {
  const mcpRequests = [];
  const mcpServer = http.createServer(async (req, res) => {
    const chunks = [];
    for await (const chunk of req) chunks.push(Buffer.from(chunk));
    ...
    if (body.method === "initialize") {
      res.writeHead(200, {
        "content-type": "application/json",
        "mcp-session-id": "sess_tool_search_mcp_test",
      });
      res.end(JSON.stringify({
        jsonrpc: "2.0",
        id: body.id,
        result: {
          protocolVersion: "2025-03-26",
          capabilities: { tools: {} },
          serverInfo: { name: "tool-search-mcp-test", version: "1.0.0" },
        },
      }));
      return;
    }
    ...
    if (body.method === "tools/list") {
      assert.equal(req.headers["mcp-session-id"], "sess_tool_search_mcp_test");
      res.writeHead(200, { "content-type": "application/json" });
      res.end(JSON.stringify({
        jsonrpc: "2.0",
        id: body.id,
        result: {
          tools: [{
            name: "roll",
            description: "Roll dice after tool_search loads this MCP schema.",
            inputSchema: {
              type: "object",
              properties: {
                diceRollExpression: { type: "string" },
    ...
    res.writeHead(500, { "content-type": "application/json" });
    res.end(JSON.stringify({ error: `unexpected tool_search MCP method: ${body.method}` }));
  });
  const mcpAddress = await listen(mcpServer);
  ...
  try {
    ...
    await withMockProvider(async (_req, res, call) => {
      ...
      if (providerCallCount === 1) {
        ...
        assert.match(prompt, /dice_server/);
        assert.equal(call.body.tools.some((tool) => /^mcp_dice_server_roll_/.test(tool.function?.name || "")), false);
        res.writeHead(200, { "content-type": "application/json" });
        res.end(JSON.stringify({
          id: "chatcmpl_tool_search_mcp_search",
          object: "chat.completion",
          created: 100,
          model: "mock-model",
          choices: [{
            ...
            message: {
              ...
              tool_calls: [{
                ...
                function: {
                  name: searchTool.function.name,
                  arguments: "{\"server_labels\":[\"dice_server\"],\"query\":\"dice roller\"}",
                },
              }],
            },
            finish_reason: "tool_calls",
      ...
      if (providerCallCount === 2) {
        ...
        const mcpTool = call.body.tools.find((tool) => /^mcp_dice_server_roll_/.test(tool.function?.name || ""));
        ...
        res.writeHead(200, { "content-type": "application/json" });
        res.end(JSON.stringify({
          id: "chatcmpl_tool_search_mcp_call",
          object: "chat.completion",
          created: 101,
          model: "mock-model",
          choices: [{
      ...
      res.writeHead(200, { "content-type": "application/json" });
      res.end(JSON.stringify({
        id: "chatcmpl_tool_search_mcp_final",
        object: "chat.completion",
        created: 102,
        model: "mock-model",
        choices: [{
          index: 0,
          message: { role: "assistant", content: "tool-search-mcp-ok" },
          finish_reason: "stop",
        }],
        usage: { prompt_tokens: 11, completion_tokens: 3, total_tokens: 14 },
      }));
      ...
      const response = await fetch(`http://127.0.0.1:${bridgeAddress.port}/v1/responses`, {
        ...
        body: JSON.stringify({
          model: "mock-model",
          input: "Find the dice MCP tool, roll 2d4+1, and return tool-search-mcp-ok.",
          tools: [
            { type: "tool_search" },
            {
              type: "mcp",
      ...
      assert.deepEqual(json.output[0].arguments.server_labels, ["dice_server"]);
      assert.equal(json.output[1].server_label, "dice_server");
      assert.equal(json.output[1].tools[0].name, "roll");
      assert.equal(json.output[1].tools[0].input_schema.properties.diceRollExpression.type, "string");
      assert.equal(json.output[2].server_label, "dice_server");
      assert.equal(json.output[2].name, "roll");
      assert.equal(json.output[2].output, "7");
      assert.equal(json.output[3].content[0].text, "tool-search-mcp-ok");
      assert.equal(json.metadata.compatibility.local_tool_search.searchable_mcp_server_count, 1);
      assert.equal(json.metadata.compatibility.local_tool_search.search_call_count, 1);
      assert.equal(json.metadata.compatibility.local_tool_search.mcp_list_tools_loaded_count, 1);
      assert.equal(json.metadata.compatibility.local_tool_search.mcp_loaded_tool_count, 1);
      assert.equal(json.metadata.compatibility.local_tool_search.boundary, "deferred_mcp_tool_search_and_load");
      assert.equal(json.metadata.compatibility.local_mcp.tool_search_list_tools_loaded_count, 1);
      assert.equal(json.metadata.compatibility.local_mcp.tool_search_loaded_tool_count, 1);
      assert.equal(json.metadata.compatibility.local_mcp.remote_import_attempt_count, 1);
      assert.equal(json.metadata.compatibility.local_mcp.remote_import_success_count, 1);
      assert.equal(json.metadata.compatibility.local_mcp.remote_call_tool_count, 1);
      assert.equal(json.metadata.compatibility.local_mcp.remote_call_success_count, 1);
      assert.equal(json.metadata.compatibility.local_mcp.boundary, "tool_search_mcp_list_tools_and_call_execution");
      assert.deepEqual(json.metadata.compatibility.local_tool_budget, {
        max_tool_calls: 2,
        used: 2,
        skipped: 0,
...
test("POST /v1/responses parses text MCP approval emitted after hosted tool_search", async () => {
  const authValue = "redaction-fixture-value-for-text-tool-search-mcp-approval";
  const mcpRequests = [];
  const mcpServer = http.createServer(async (req, res) => {
    const chunks = [];
    for await (const chunk of req) chunks.push(Buffer.from(chunk));
    ...
    if (body.method === "initialize") {
      ...
      res.writeHead(200, {
        "content-type": "application/json",
        "mcp-session-id": "sess_text_tool_search_mcp_approval_test",
      });
      res.end(JSON.stringify({
        jsonrpc: "2.0",
        id: body.id,
        result: {
          protocolVersion: "2025-03-26",
          capabilities: { tools: {} },
          serverInfo: { name: "text-tool-search-mcp-approval-test", version: "1.0.0" },
        },
      }));
      return;
    }
    ...
    if (body.method === "tools/list") {
      assert.equal(req.headers.authorization, `Bearer ${authValue}`);
      assert.equal(req.headers["mcp-session-id"], "sess_text_tool_search_mcp_approval_test");
      res.writeHead(200, { "content-type": "application/json" });
      res.end(JSON.stringify({
        jsonrpc: "2.0",
        id: body.id,
        result: {
          tools: [{
            name: "roll",
            description: "Roll dice after text approval.",
            inputSchema: {
              type: "object",
              properties: {
                diceRollExpression: { type: "string" },
    ...
    res.writeHead(500, { "content-type": "application/json" });
    res.end(JSON.stringify({ error: `unexpected text approval MCP method: ${body.method}` }));
  });
  const mcpAddress = await listen(mcpServer);
  ...
  try {
    ...
    await withMockProvider(async (_req, res, call) => {
      ...
      if (providerCallCount === 1) {
        ...
        res.writeHead(200, { "content-type": "application/json" });
        res.end(JSON.stringify({
          id: "chatcmpl_text_tool_search_mcp_approval_search",
          object: "chat.completion",
          created: 100,
          model: "mock-model",
          choices: [{
            ...
            message: {
              ...
              content: null,
              tool_calls: [{
                id: "call_search_text_approval_mcp",
                type: "function",
                function: {
                  name: searchTool.function.name,
                  arguments: "{\"server_labels\":[\"approval_text_dice\"],\"query\":\"approval dice roller\"}",
                },
              }],
            },
            finish_reason: "tool_calls",
      ...
      assert.equal(providerCallCount, 2);
      assert.ok(call.body.tools.some((tool) => /^mcp_approval_text_dice_roll_/.test(tool.function?.name || "")));
      res.writeHead(200, { "content-type": "application/json" });
      res.end(JSON.stringify({
        id: "chatcmpl_text_tool_search_mcp_approval_request",
        object: "chat.completion",
        created: 101,
        model: "mock-model",
        choices: [{
          ...
          message: {
            ...
            content: [
              "<｜｜DSML｜｜tool_calls>",
              "<｜｜DSML｜｜invoke name=\"mcp_tool_approval_request\">",
              "<｜｜DSML｜｜parameter name=\"requests\" string=\"false\">[[\"approval_text_dice\",\"roll\",{\"diceRollExpression\":\"2d4+1\"}]]</｜｜DSML｜｜parameter>",
              "</｜｜DSML｜｜invoke>",
              "</｜｜DSML｜｜tool_calls>",
            ].join("\n"),
          },
      ...
      const response = await fetch(`http://127.0.0.1:${bridgeAddress.port}/v1/responses`, {
        ...
        body: JSON.stringify({
          ...
          tools: [
            ...
            {
              type: "mcp",
              server_label: "approval_text_dice",
              server_description: "Dice roller server requiring text approval.",
              server_url: `http://127.0.0.1:${mcpAddress.port}/mcp`,
              authorization: authValue,
              require_approval: "always",
              allowed_tools: ["roll"],
      ...
      assert.equal(json.output[2].server_label, "approval_text_dice");
      assert.equal(json.output[2].name, "roll");
      assert.equal(json.output[2].arguments, "{\"diceRollExpression\":\"2d4+1\"}");
      assert.equal(json.metadata.compatibility.local_mcp.remote_text_tool_call_count, 1);
      assert.equal(json.metadata.compatibility.local_mcp.remote_approval_request_count, 1);
      assert.equal(json.metadata.compatibility.local_mcp.remote_call_attempt_count, 0);
      assert.equal(json.metadata.compatibility.local_mcp.boundary, "tool_search_mcp_list_tools_with_approval_request");
      const serialized = JSON.stringify(json);
      assert.doesNotMatch(serialized, /DSML/);
      assert.doesNotMatch(serialized, new RegExp(authValue));
    }, { mcpRemoteListTools: true, mcpRemoteToolCalls: true, mcpTimeoutMs: 1000, mcpMaxCallRounds: 2 });
  } finally {
    await close(mcpServer);
  }
...
test("local MCP suppresses pseudo tool markup after a successful remote call", () => {
  const context = {
    remote_call_success_count: 1,
  };
  const response = {
    output: [{
      id: "msg_pseudo_mcp",
      ...
      content: [{
        ...
        text: [
          "<｜｜DSML｜｜tool_calls>",
          "<｜｜DSML｜｜invoke name=\"mcp_approval_request\">",
          "<｜｜DSML｜｜parameter name=\"server_label\">remote_eval</｜｜DSML｜｜parameter>",
          "</｜｜DSML｜｜invoke>",
          "</｜｜DSML｜｜tool_calls>",
        ].join("\n"),
      }],
  ...
  assert.equal(context.remote_text_suppressed_count, 1);
});

test("POST /v1/responses streams approval flow for deferred remote MCP loaded through hosted tool_search", async () => {
  const authValue = "redaction-fixture-value-for-stream-tool-search-mcp-approval";
  const mcpRequests = [];
  const mcpServer = http.createServer(async (req, res) => {
    const chunks = [];
    for await (const chunk of req) chunks.push(Buffer.from(chunk));
    ...
    if (body.method === "initialize") {
      ...
      res.writeHead(200, {
        "content-type": "application/json",
        "mcp-session-id": "sess_stream_tool_search_mcp_approval_test",
      });
      res.end(JSON.stringify({
        jsonrpc: "2.0",
        id: body.id,
        result: {
          protocolVersion: "2025-03-26",
          capabilities: { tools: {} },
          serverInfo: { name: "stream-tool-search-mcp-approval-test", version: "1.0.0" },
        },
      }));
      return;
    }
    ...
    if (body.method === "tools/list") {
      assert.equal(req.headers.authorization, `Bearer ${authValue}`);
      assert.equal(req.headers["mcp-session-id"], "sess_stream_tool_search_mcp_approval_test");
      res.writeHead(200, { "content-type": "application/json" });
      res.end(JSON.stringify({
        jsonrpc: "2.0",
        id: body.id,
        result: {
          tools: [{
            name: "roll",
            description: "Roll dice after streaming tool_search loads this approval-required MCP schema.",
            inputSchema: {
              type: "object",
              properties: {
                diceRollExpression: { type: "string" },
    ...
    res.writeHead(500, { "content-type": "application/json" });
    res.end(JSON.stringify({ error: `unexpected streaming approval tool_search MCP method: ${body.method}` }));
  });
  const mcpAddress = await listen(mcpServer);
  ...
  try {
    ...
    await withMockProvider(async (_req, res, call) => {
      ...
      if (providerCallCount === 1) {
        ...
        assert.match(searchTool.function.name, /^local_tool_search/);
        assert.match(prompt, /Searchable MCP servers:/);
        assert.match(prompt, /stream_approval_dice/);
        assert.equal(call.body.tools.some((tool) => /^mcp_stream_approval_dice_roll_/.test(tool.function?.name || "")), false);
        res.writeHead(200, { "content-type": "text/event-stream" });
        res.write(`data: ${JSON.stringify({
          id: "chatcmpl_stream_tool_search_mcp_approval_search",
          object: "chat.completion.chunk",
          created: 100,
          model: "mock-model",
          choices: [{
            ...
            delta: {
              ...
              tool_calls: [{
                index: 0,
                id: "call_stream_search_approval_mcp",
                type: "function",
                function: { name: searchTool.function.name, arguments: "" },
              }],
            },
        ...
        res.write(`data: ${JSON.stringify({
          ...
          choices: [{
            ...
            delta: {
              tool_calls: [{
                index: 0,
                function: { arguments: "{\"server_labels\":[\"stream_approval_dice\"],\"query\":\"approval dice roller\"}" },
              }],
            },
            finish_reason: null,
          }],
      ...
      if (providerCallCount === 2) {
        ...
        assert.ok(toolMessage);
        assert.equal(JSON.parse(toolMessage.content).mcp_loaded_tool_count, 1);
        const mcpTool = call.body.tools.find((tool) => /^mcp_stream_approval_dice_roll_/.test(tool.function?.name || ""));
        assert.ok(mcpTool, JSON.stringify(call.body.tools.map((tool) => tool.function?.name || "")));
        mcpChatToolName = mcpTool.function.name;
        assert.equal(mcpTool.function.parameters.properties.diceRollExpression.type, "string");
        res.writeHead(200, { "content-type": "text/event-stream" });
        res.write(`data: ${JSON.stringify({
          id: "chatcmpl_stream_tool_search_mcp_approval_request",
          object: "chat.completion.chunk",
          created: 101,
          model: "mock-model",
          choices: [{
            ...
            delta: {
              ...
              tool_calls: [{
                index: 0,
                id: "call_stream_tool_search_approval_roll",
                type: "function",
      ...
      res.writeHead(200, { "content-type": "text/event-stream" });
      res.write(`data: ${JSON.stringify({
        id: "chatcmpl_stream_tool_search_mcp_approval_final",
        object: "chat.completion.chunk",
        created: 102,
        model: "mock-model",
        choices: [{
          index: 0,
          delta: { role: "assistant", content: "stream-approved-tool-search-mcp-ok" },
          finish_reason: null,
        }],
      })}\n\n`);
      res.write(`data: ${JSON.stringify({
      ...
      const mcpTool = {
        type: "mcp",
        server_label: "stream_approval_dice",
        server_description: "Streaming dice roller server requiring approval.",
        server_url: `http://127.0.0.1:${mcpAddress.port}/mcp`,
        authorization: authValue,
        require_approval: "always",
        allowed_tools: ["roll"],
      ...
      const firstResponse = await fetch(`${baseUrl}/v1/responses`, {
        ...
        body: JSON.stringify({
          model: "mock-model",
          input: "Stream-search the approval dice MCP tool and request a roll of 2d4+1.",
          stream: true,
          tools: [
            { type: "tool_search" },
            mcpTool,
      ...
      assert.equal(requests.length, 2);
      assert.ok(!firstEvents.some((event) => event.data?.item?.type === "function_call"));
      assert.ok(!firstEvents.some((event) => event.event === "response.function_call_arguments.delta"));
      assert.ok(!firstEvents.some((event) => event.event === "response.mcp_call.in_progress"));
      assert.ok(firstEvents.some((event) => event.event === "response.output_item.added"
        && event.data.item?.type === "tool_search_call"));
      assert.ok(firstEvents.some((event) => event.event === "response.output_item.added"
        && event.data.item?.type === "mcp_list_tools"));
      const approvalAdded = firstEvents.find((event) => event.event === "response.output_item.added"
        && event.data.item?.type === "mcp_approval_request");
      assert.ok(approvalAdded);
      assert.equal(approvalAdded.data.item.server_label, "stream_approval_dice");
      assert.equal(approvalAdded.data.item.name, "roll");
      assert.equal(approvalAdded.data.item.arguments, "{\"diceRollExpression\":\"2d4+1\"}");
      const first = firstEvents.find((event) => event.event === "response.completed").data.response;
      assert.deepEqual(first.output.map((item) => item.type), [
        "tool_search_call",
        "mcp_list_tools",
        "mcp_approval_request",
      ]);
      assert.deepEqual(first.output[0].arguments.server_labels, ["stream_approval_dice"]);
      assert.equal(first.output[1].server_label, "stream_approval_dice");
      assert.equal(first.output[1].tools[0].name, "roll");
      assert.equal(first.output[2].server_label, "stream_approval_dice");
      assert.equal(first.output[2].name, "roll");
      assert.equal(first.output[2].arguments, "{\"diceRollExpression\":\"2d4+1\"}");
      assert.equal(first.usage.input_tokens, 18);
      assert.equal(first.usage.output_tokens, 4);
      assert.equal(first.usage.total_tokens, 22);
      assert.equal(first.metadata.compatibility.local_tool_search.mcp_list_tools_loaded_count, 1);
      assert.equal(first.metadata.compatibility.local_tool_search.mcp_loaded_tool_count, 1);
      assert.equal(first.metadata.compatibility.local_mcp.tool_search_list_tools_loaded_count, 1);
      assert.equal(first.metadata.compatibility.local_mcp.remote_approval_request_count, 1);
      ...
      assert.equal(requests.length, 3);
      assert.ok(!secondEvents.some((event) => event.data?.item?.type === "function_call"));
      assert.ok(!secondEvents.some((event) => event.event === "response.function_call_arguments.delta"));
      assert.ok(secondEvents.some((event) => event.event === "response.mcp_call_arguments.delta"
        && event.data.delta === "{\"diceRollExpression\":\"2d4+1\"}"));
      assert.ok(secondEvents.some((event) => event.event === "response.mcp_call_arguments.done"
        && event.data.arguments === "{\"diceRollExpression\":\"2d4+1\"}"));
      assert.ok(secondEvents.some((event) => event.event === "response.mcp_call.in_progress"));
      const second = secondEvents.find((event) => event.event === "response.completed").data.response;
      assert.deepEqual(second.output.map((item) => item.type), ["mcp_call", "message"]);
      assert.equal(second.output[0].approval_request_id, approval.id);
      assert.equal(second.output[0].server_label, "stream_approval_dice");
      assert.equal(second.output[0].name, "roll");
      assert.equal(second.output[0].arguments, "{\"diceRollExpression\":\"2d4+1\"}");
      assert.equal(second.output[0].output, "7");
      assert.equal(second.output[0].error, null);
      assert.equal(second.output[1].content[0].text, "stream-approved-tool-search-mcp-ok");
      assert.equal(second.usage.input_tokens, 11);
      assert.equal(second.usage.output_tokens, 4);
      assert.equal(second.usage.total_tokens, 15);
      assert.equal(second.metadata.compatibility.local_mcp.input_list_tools_loaded_count, 1);
      assert.equal(second.metadata.compatibility.local_mcp.remote_import_attempt_count, 0);
      assert.equal(second.metadata.compatibility.local_mcp.remote_approval_response_count, 1);
      assert.equal(second.metadata.compatibility.local_mcp.remote_approval_approved_count, 1);
      ...
      assert.match(secondEvents.map((event) => event.data?.delta || "").join(""), /stream-approved-tool-search-mcp-ok/);
      assert.doesNotMatch(JSON.stringify(second), new RegExp(authValue));
    }, { mcpRemoteListTools: true, mcpRemoteToolCalls: true, mcpTimeoutMs: 1000, mcpMaxCallRounds: 2 });
  } finally {
    await close(mcpServer);
...
test("POST /v1/responses executes auto-approved remote MCP tools/call through Chat tool calls", async () => {
  ...
  const mcpServer = http.createServer(async (req, res) => {
    ...
    if (body.method === "initialize") {
      res.writeHead(200, {
        "content-type": "application/json",
        "mcp-session-id": "sess_remote_mcp_call_test",
      });
      res.end(JSON.stringify({
        jsonrpc: "2.0",
        id: body.id,
        result: {
          protocolVersion: "2025-03-26",
          capabilities: { tools: {} },
          serverInfo: { name: "remote-mcp-call-test", version: "1.0.0" },
        },
      }));
      return;
    }
    ...
    if (body.method === "tools/call") {
      assert.equal(req.headers["mcp-session-id"], "sess_remote_mcp_call_test");
      assert.equal(body.params.name, "roll");
      assert.deepEqual(body.params.arguments, { diceRollExpression: "2d4+1" });
      res.writeHead(200, { "content-type": "application/json" });
      res.end(JSON.stringify({
  ...
  try {
    await withMockProvider(async (_req, res, call) => {
      if (call.body.messages.some((message) => message.role === "tool")) {
        ...
        assert.match(toolMessage.content, /^7/);
        assert.match(toolMessage.content, /Bearer \[redacted\]/);
        assert.doesNotMatch(JSON.stringify(call.body), new RegExp(authValue));
        res.writeHead(200, { "content-type": "application/json" });
        res.end(JSON.stringify({
          id: "chatcmpl_remote_mcp_final",
          object: "chat.completion",
          created: 101,
          model: "mock-model",
          choices: [{
            index: 0,
            message: { role: "assistant", content: "remote-call-ok" },
            finish_reason: "stop",
          }],
          usage: { prompt_tokens: 11, completion_tokens: 2, total_tokens: 13 },
        }));
      ...
      const response = await fetch(`http://127.0.0.1:${bridgeAddress.port}/v1/responses`, {
        ...
        body: JSON.stringify({
          model: "mock-model",
          input: "Roll 2d4+1 with remote MCP and return remote-call-ok.",
          tools: [{
            type: "mcp",
            server_label: "remote_call",
            server_url: `http://127.0.0.1:${mcpAddress.port}/mcp`,
            authorization: authValue,
            require_approval: "never",
            allowed_tools: ["roll"],
      ...
      assert.equal(requests.length, 2);
      assert.equal(json.output[0].type, "mcp_list_tools");
      assert.equal(json.output[1].type, "mcp_call");
      assert.equal(json.output[1].server_label, "remote_call");
      assert.equal(json.output[1].name, "roll");
      assert.equal(json.output[1].arguments, "{\"diceRollExpression\":\"2d4+1\"}");
      assert.match(json.output[1].output, /^7/);
      assert.match(json.output[1].output, /Bearer \[redacted\]/);
      assert.equal(json.output[1].error, null);
      assert.equal(json.output[2].content[0].text, "remote-call-ok");
      assert.equal(json.usage.input_tokens, 18);
      assert.equal(json.usage.output_tokens, 3);
      assert.equal(json.usage.total_tokens, 21);
      assert.equal(json.metadata.compatibility.local_mcp.remote_call_tool_count, 1);
      assert.equal(json.metadata.compatibility.local_mcp.remote_call_attempt_count, 1);
      assert.equal(json.metadata.compatibility.local_mcp.remote_call_success_count, 1);
...
test("POST /v1/responses streams auto-approved remote MCP tools/call through Chat tool calls", async () => {
  const authValue = "redaction-fixture-value-for-stream-remote-mcp-call";
  const mcpRequests = [];
  const mcpServer = http.createServer(async (req, res) => {
    const chunks = [];
    for await (const chunk of req) chunks.push(Buffer.from(chunk));
    ...
    if (body.method === "initialize") {
      res.writeHead(200, {
        "content-type": "application/json",
        "mcp-session-id": "sess_stream_remote_mcp_call_test",
      });
      res.end(JSON.stringify({
        jsonrpc: "2.0",
        id: body.id,
        result: {
          protocolVersion: "2025-03-26",
          capabilities: { tools: {} },
          serverInfo: { name: "stream-remote-mcp-call-test", version: "1.0.0" },
        },
      }));
      return;
    }
    ...
    if (body.method === "tools/list") {
      assert.equal(req.headers["mcp-session-id"], "sess_stream_remote_mcp_call_test");
      res.writeHead(200, { "content-type": "application/json" });
      res.end(JSON.stringify({
        jsonrpc: "2.0",
        id: body.id,
        result: {
          tools: [{
            name: "roll",
            description: "Roll dice remotely while streaming",
            inputSchema: {
              type: "object",
              properties: {
                diceRollExpression: { type: "string" },
  ...
  try {
    await withMockProvider(async (_req, res, call) => {
      ...
      if (call.body.messages.some((message) => message.role === "tool")) {
        ...
        res.writeHead(200, { "content-type": "text/event-stream" });
        res.write(`data: ${JSON.stringify({
          id: "chatcmpl_stream_remote_mcp_final",
          object: "chat.completion.chunk",
          created: 101,
          model: "mock-model",
          choices: [{
            index: 0,
            delta: { role: "assistant", content: "stream-remote-call-ok" },
            finish_reason: null,
          }],
        })}\n\n`);
        res.write(`data: ${JSON.stringify({
      ...
      assert.equal(call.body.tools.length, 1);
      const toolName = call.body.tools[0].function.name;
      assert.match(toolName, /^mcp_stream_remote_call_roll_/);
      assert.equal(call.body.tools[0].function.parameters.properties.diceRollExpression.type, "string");
      ...
      res.writeHead(200, { "content-type": "text/event-stream" });
      res.write(`data: ${JSON.stringify({
        id: "chatcmpl_stream_remote_mcp_call",
        object: "chat.completion.chunk",
        created: 100,
        model: "mock-model",
        choices: [{
      ...
      const response = await fetch(`http://127.0.0.1:${bridgeAddress.port}/v1/responses`, {
        ...
        body: JSON.stringify({
          model: "mock-model",
          input: "Stream roll 2d4+1 with remote MCP and return stream-remote-call-ok.",
          stream: true,
          tools: [{
            type: "mcp",
            server_label: "stream_remote_call",
            server_url: `http://127.0.0.1:${mcpAddress.port}/mcp`,
            authorization: authValue,
            require_approval: "never",
            allowed_tools: ["roll"],
      ...
      assert.equal(completed.output[1].type, "mcp_call");
      assert.equal(completed.output[1].server_label, "stream_remote_call");
      assert.equal(completed.output[1].name, "roll");
      assert.equal(completed.output[1].arguments, "{\"diceRollExpression\":\"2d4+1\"}");
      assert.match(completed.output[1].output, /^7/);
      assert.match(completed.output[1].output, /Bearer \[redacted\]/);
      assert.equal(completed.output[1].error, null);
      assert.equal(completed.output[2].content[0].text, "stream-remote-call-ok");
      assert.equal(completed.usage.input_tokens, 18);
      assert.equal(completed.usage.output_tokens, 4);
      assert.equal(completed.usage.total_tokens, 22);
      assert.equal(completed.metadata.compatibility.local_mcp.remote_call_tool_count, 1);
      assert.equal(completed.metadata.compatibility.local_mcp.remote_call_attempt_count, 1);
      assert.equal(completed.metadata.compatibility.local_mcp.remote_call_success_count, 1);
      assert.equal(completed.metadata.compatibility.local_mcp.remote_call_failed_count, 0);
      assert.equal(completed.metadata.compatibility.local_mcp.boundary, "remote_list_tools_and_call_execution");
      assert.deepEqual(completed.metadata.compatibility.local_tool_budget, {
        max_tool_calls: 2,
        used: 2,
        skipped: 0,
      ...
      assert.match(events.map((event) => event.data?.delta || "").join(""), /stream-remote-call-ok/);
      assert.doesNotMatch(JSON.stringify(completed), new RegExp(authValue));
    }, { mcpRemoteListTools: true, mcpRemoteToolCalls: true, mcpTimeoutMs: 1000 });
  } finally {
    await close(mcpServer);
  }
...
test("POST /v1/responses background executes auto-approved remote MCP tools/call", async () => {
  ...
  try {
    await withMockProvider(async (_req, res, call) => {
      ...
      assert.equal(finalJson.status, "completed", JSON.stringify(finalJson.error || finalJson.metadata || finalJson));
      assert.equal(requests.length, 2);
...
test("POST /v1/responses streams remote MCP approval requests without executing tools/call", async () => {
  const authValue = "redaction-fixture-value-for-stream-remote-mcp-approval";
  const mcpRequests = [];
  const mcpServer = http.createServer(async (req, res) => {
    const chunks = [];
    for await (const chunk of req) chunks.push(Buffer.from(chunk));
    ...
    if (body.method === "initialize") {
      res.writeHead(200, {
        "content-type": "application/json",
        "mcp-session-id": "sess_stream_remote_mcp_approval_test",
      });
      res.end(JSON.stringify({
        jsonrpc: "2.0",
        id: body.id,
        result: {
          protocolVersion: "2025-03-26",
          capabilities: { tools: {} },
          serverInfo: { name: "stream-remote-mcp-approval-test", version: "1.0.0" },
        },
      }));
      return;
    }
    ...
    if (body.method === "tools/list") {
      assert.equal(req.headers["mcp-session-id"], "sess_stream_remote_mcp_approval_test");
      res.writeHead(200, { "content-type": "application/json" });
      res.end(JSON.stringify({
        jsonrpc: "2.0",
        id: body.id,
        result: {
          tools: [{
            name: "roll",
            description: "Roll dice after streaming approval",
            inputSchema: {
              type: "object",
              properties: {
                diceRollExpression: { type: "string" },
  ...
  try {
    await withMockProvider(async (_req, res, call) => {
      ...
      assert.equal(call.body.tools.length, 1);
      const toolName = call.body.tools[0].function.name;
      assert.match(toolName, /^mcp_stream_remote_approval_roll_/);
      assert.equal(call.body.tool_choice, "auto");
      assert.doesNotMatch(JSON.stringify(call.body), new RegExp(authValue));
      res.writeHead(200, { "content-type": "text/event-stream" });
      res.write(`data: ${JSON.stringify({
        id: "chatcmpl_stream_remote_mcp_approval",
        object: "chat.completion.chunk",
        created: 100,
        model: "mock-model",
        choices: [{
          ...
          delta: {
            ...
            tool_calls: [{
              index: 0,
              id: "call_roll_remote_stream_approval",
              type: "function",
              function: { name: toolName, arguments: "" },
            }],
          },
      ...
      const response = await fetch(`http://127.0.0.1:${bridgeAddress.port}/v1/responses`, {
        ...
        body: JSON.stringify({
          model: "mock-model",
          input: "Stream a request for approval to roll 2d4+1.",
          stream: true,
          tools: [{
            type: "mcp",
            server_label: "stream_remote_approval",
            server_url: `http://127.0.0.1:${mcpAddress.port}/mcp`,
            authorization: authValue,
            require_approval: "always",
            allowed_tools: ["roll"],
      ...
      const added = events.find((event) => event.event === "response.output_item.added"
        && event.data.item?.type === "mcp_approval_request");
      assert.ok(added);
      assert.equal(added.data.item.server_label, "stream_remote_approval");
      assert.equal(added.data.item.name, "roll");
      assert.equal(added.data.item.arguments, "{\"diceRollExpression\":\"2d4+1\"}");
      const completed = events.find((event) => event.event === "response.completed").data.response;
      assert.equal(completed.output[0].type, "mcp_list_tools");
      assert.equal(completed.output[1].type, "mcp_approval_request");
      assert.equal(completed.output[1].server_label, "stream_remote_approval");
      assert.equal(completed.output[1].name, "roll");
      assert.equal(completed.output[1].arguments, "{\"diceRollExpression\":\"2d4+1\"}");
      assert.equal(completed.metadata.compatibility.local_mcp.remote_approval_request_count, 1);
      assert.equal(completed.metadata.compatibility.local_mcp.remote_call_attempt_count, 0);
      assert.equal(completed.metadata.compatibility.local_mcp.boundary, "remote_list_tools_with_approval_request");
      assert.deepEqual(completed.metadata.compatibility.local_tool_budget, {
        max_tool_calls: 2,
        used: 2,
        skipped: 0,
...
test("POST /v1/responses streams approved remote MCP approval response execution", async () => {
  const authValue = "redaction-fixture-value-for-stream-remote-mcp-approval-response";
  const mcpRequests = [];
  const mcpServer = http.createServer(async (req, res) => {
    const chunks = [];
    for await (const chunk of req) chunks.push(Buffer.from(chunk));
    ...
    if (body.method === "initialize") {
      res.writeHead(200, {
        "content-type": "application/json",
        "mcp-session-id": "sess_stream_remote_mcp_approval_response_test",
      });
      res.end(JSON.stringify({
        jsonrpc: "2.0",
        id: body.id,
        result: {
          protocolVersion: "2025-03-26",
          capabilities: { tools: {} },
          serverInfo: { name: "stream-remote-mcp-approval-response-test", version: "1.0.0" },
        },
      }));
      return;
    }
    ...
    if (body.method === "tools/list") {
      assert.equal(req.headers["mcp-session-id"], "sess_stream_remote_mcp_approval_response_test");
      res.writeHead(200, { "content-type": "application/json" });
      res.end(JSON.stringify({
        jsonrpc: "2.0",
        id: body.id,
        result: {
          tools: [{
            name: "roll",
            description: "Roll dice after streaming approval response",
            inputSchema: {
              type: "object",
              properties: {
                diceRollExpression: { type: "string" },
  ...
  try {
    await withMockProvider(async (_req, res, call) => {
      ...
      if (/MCP call output items produced by the bridge/.test(prompt)) {
        ...
        res.writeHead(200, { "content-type": "text/event-stream" });
        res.write(`data: ${JSON.stringify({
          id: "chatcmpl_stream_remote_mcp_approval_response_final",
          object: "chat.completion.chunk",
          created: 102,
          model: "mock-model",
          choices: [{
            index: 0,
            delta: { role: "assistant", content: "stream-approved-call-ok" },
            finish_reason: null,
          }],
        })}\n\n`);
        res.write(`data: ${JSON.stringify({
      ...
      assert.equal(call.body.tools.length, 1);
      const toolName = call.body.tools[0].function.name;
      assert.match(toolName, /^mcp_stream_remote_approval_respons_roll_/);
      assert.equal(call.body.tool_choice, "auto");
      assert.doesNotMatch(JSON.stringify(call.body), new RegExp(authValue));
      res.writeHead(200, { "content-type": "text/event-stream" });
      res.write(`data: ${JSON.stringify({
        id: "chatcmpl_stream_remote_mcp_approval_response_request",
        object: "chat.completion.chunk",
        created: 101,
        model: "mock-model",
        choices: [{
          ...
          delta: {
            ...
            tool_calls: [{
              index: 0,
              id: "call_roll_remote_stream_approval_response",
              type: "function",
              function: { name: toolName, arguments: "" },
            }],
          },
      ...
      const tool = {
        type: "mcp",
        server_label: "stream_remote_approval_response",
        server_url: `http://127.0.0.1:${mcpAddress.port}/mcp`,
        authorization: authValue,
        require_approval: "always",
        allowed_tools: ["roll"],
      ...
      const firstEvents = parseSseEvents(await firstResponse.text());
      const first = firstEvents.find((event) => event.event === "response.completed").data.response;
      const approval = first.output.find((item) => item.type === "mcp_approval_request");
      assert.ok(approval);
      assert.equal(requests.length, 1);
      assert.equal(first.metadata.compatibility.local_mcp.remote_approval_request_count, 1);
      assert.doesNotMatch(JSON.stringify(first), new RegExp(authValue));
      ...
      assert.equal(second.output[1].type, "mcp_call");
      assert.equal(second.output[1].approval_request_id, approval.id);
      assert.equal(second.output[1].server_label, "stream_remote_approval_response");
      assert.equal(second.output[1].name, "roll");
      assert.equal(second.output[1].arguments, "{\"diceRollExpression\":\"2d4+1\"}");
      assert.equal(second.output[1].output, "7");
      assert.equal(second.output[1].error, null);
      assert.equal(second.output[2].content[0].text, "stream-approved-call-ok");
      assert.equal(second.metadata.compatibility.local_mcp.remote_approval_response_count, 1);
      assert.equal(second.metadata.compatibility.local_mcp.remote_approval_approved_count, 1);
      assert.equal(second.metadata.compatibility.local_mcp.remote_call_success_count, 1);
      assert.equal(second.metadata.compatibility.local_mcp.boundary, "remote_list_tools_and_call_execution");
      ...
      assert.match(secondEvents.map((event) => event.data?.delta || "").join(""), /stream-approved-call-ok/);
      assert.doesNotMatch(JSON.stringify(second), new RegExp(authValue));
    }, { mcpRemoteListTools: true, mcpRemoteToolCalls: true, mcpTimeoutMs: 1000 });
  } finally {
    await close(mcpServer);
...
test("POST /v1/images/variations accepts multipart placeholder requests", async () => {
  const sourcePng = "iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR42mP8/x8AAwMCAO+/p9sAAAAASUVORK5CYII=";
  await withMockProvider(async () => {
    assert.fail("chat provider should not be called for placeholder image variations");
  }, async ({ bridgeAddress, requests }) => {
    const form = new FormData();
    ...
    const response = await fetch(`http://127.0.0.1:${bridgeAddress.port}/v1/images/variations`, {
      method: "POST",
      body: form,
    });
    assert.equal(response.status, 200);
    ...
    assert.deepEqual(Buffer.from(json.data[1].b64_json, "base64").subarray(0, 8), Buffer.from("89504e470d0a1a0a", "hex"));
    assert.notEqual(json.data[0].b64_json, json.data[1].b64_json);
    assert.match(json.data[0].revised_prompt, /variation of the supplied image/);

    const missing = await fetch(`http://127.0.0.1:${bridgeAddress.port}/v1/images/variations`, {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({ model: "dall-e-2", n: 1 }),
    });
    assert.equal(missing.status, 400);
    const error = await missing.json();
    assert.equal(error.error.code, "missing_required_parameter");
    assert.equal(error.error.param, "image");
    assert.equal(requests.length, 0);
  }, {
    imageGenerationPlaceholderSize: 16,
  });
...
test("POST /v1/images/variations can call an OpenAI-compatible Images API", async () => {
  ...
  await withMockProvider(async (req, res, call) => {
    assert.equal(req.url, "/images/variations");
    ...
    res.end(JSON.stringify({
      created: 100,
      data: [
        { b64_json: variationPng },
        { b64_json: variationPng },
      ],
    }));
  }, async ({ bridgeAddress, requests }) => {
    const form = new FormData();
    form.append("model", "direct-variation-model");
    form.append("n", "2");
    form.append("size", "512x512");
    form.append("response_format", "b64_json");
    form.append("user", "direct-variation-user");
    form.append("image", new Blob([Buffer.from(sourcePng, "base64")], { type: "image/png" }), "source.png");
    ...
    assert.equal(response.status, 200);
    const json = await response.json();
    assert.equal(json.created, 100);
    assert.equal(json.data.length, 2);
    assert.equal(json.data[0].b64_json, variationPng);
    assert.equal(requests.length, 1);
  }, ({ providerAddress }) => ({
    imageGenerationProvider: "openai-compatible",
    imageGenerationBaseUrl: `http://127.0.0.1:${providerAddress.port}`,
    imageGenerationVariationPath: "/images/variations",
    imageGenerationApiKey: "image-test-key",
    imageGenerationVariationModel: "dall-e-2",
  }));
});
...
test("POST /v1/responses/input_tokens validates seed before provider calls", async () => {
  await withMockProvider(async (_req, res, call) => {
    assert.equal(call.body.seed, -7);
    assert.equal(call.body.max_tokens, 1);
    res.writeHead(200, { "content-type": "application/json" });
    res.end(JSON.stringify({
      id: "chatcmpl_input_seed_probe",
      object: "chat.completion",
      created: 100,
      model: "mock-model",
      choices: [{
      ...
      usage: { prompt_tokens: 56, completion_tokens: 1, total_tokens: 57 },
    }));
  }, async ({ bridgeAddress, requests }) => {
    const baseUrl = `http://127.0.0.1:${bridgeAddress.port}`;
    for (const seed of ["7", 7.5, [], {}, -1e21]) {
      const response = await fetch(`${baseUrl}/v1/responses/input_tokens`, {
        method: "POST",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({
          model: "mock-model",
          input: "Count invalid seed request.",
          seed,
        }),
      });
      assert.equal(response.status, 400);
      assert.deepEqual(await response.json(), {
        error: {
          message: Number.isInteger(seed)
            ? "seed must be an integer between -9223372036854776000 and 9223372036854776000"
    ...
    const valid = await fetch(`${baseUrl}/v1/responses/input_tokens`, {
      ...
      body: JSON.stringify({
        model: "mock-model",
        input: "Count valid seed request.",
        seed: -7,
      }),
    });
    assert.equal(valid.status, 200);
...
test("Organization roles and groups manage local memberships and assignments", async () => {
  await withMockProvider(async () => {
    ...
    const roleResponse = await fetch(`${baseUrl}/v1/organization/roles`, {
      ...
      headers: { "content-type": "application/json" },
      body: JSON.stringify({
        role_name: "Bridge Group Manager",
        description: "Manage local compatibility groups",
        permissions: ["api.groups.read", "api.groups.write", "api.groups.read"],
      }),
    });
    assert.equal(roleResponse.status, 200);
    const role = await roleResponse.json();
    assert.equal(role.object, "role");
    assert.match(role.id, /^role_/);
    assert.equal(role.name, "Bridge Group Manager");
    assert.deepEqual(role.permissions, ["api.groups.read", "api.groups.write"]);
    ...
    assert.equal((await fetchedRole.json()).description, "Manage local compatibility groups");

    const updatedRole = await fetch(`${baseUrl}/v1/organization/roles/${role.id}`, {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({
        role_name: "Bridge Group Operator",
        permissions: ["api.groups.read"],
        description: null,
      }),
    });
    assert.equal(updatedRole.status, 200);
    const updatedRoleJson = await updatedRole.json();
    assert.equal(updatedRoleJson.name, "Bridge Group Operator");
    assert.equal(updatedRoleJson.description, null);
    assert.deepEqual(updatedRoleJson.permissions, ["api.groups.read"]);

    const invalidRole = await fetch(`${baseUrl}/v1/organization/roles`, {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({ role_name: "Invalid Role", permissions: [] }),
    });
    assert.equal(invalidRole.status, 400);
    assert.equal((await invalidRole.json()).error.code, "invalid_role_permissions");

    const groupResponse = await fetch(`${baseUrl}/v1/organization/groups`, {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({ name: "Bridge Support Team" }),
    });
    assert.equal(groupResponse.status, 200);
    const group = await groupResponse.json();
    assert.equal(group.object, "group");
    assert.match(group.id, /^group_/);
    assert.equal(group.name, "Bridge Support Team");
    ...
    assert.equal((await fetchedGroup.json()).name, "Bridge Support Team");

    const updatedGroup = await fetch(`${baseUrl}/v1/organization/groups/${group.id}`, {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({ name: "Bridge Support Ops" }),
    });
    assert.equal(updatedGroup.status, 200);
    assert.equal((await updatedGroup.json()).name, "Bridge Support Ops");

    const projectResponse = await fetch(`${baseUrl}/v1/organization/projects`, {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({ name: "Bridge Role Project" }),
    });
    assert.equal(projectResponse.status, 200);
    const project = await projectResponse.json();
    ...
    const projectUserResponse = await fetch(`${baseUrl}/v1/organization/projects/${project.id}/users`, {
      ...
      headers: { "content-type": "application/json" },
      body: JSON.stringify({
        user_id: "user_bridge_role_member",
        email: "bridge-role-member@example.com",
        name: "Bridge Role Member",
        role: "member",
      }),
    });
    assert.equal(projectUserResponse.status, 200);
    ...
    const groupUser = await fetch(`${baseUrl}/v1/organization/groups/${group.id}/users`, {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({ user_id: projectUser.id }),
    });
    assert.equal(groupUser.status, 200);
    assert.deepEqual(await groupUser.json(), {
      object: "group.user",
      group_id: group.id,
    ...
    const fetchedGroupUserJson = await fetchedGroupUser.json();
    assert.equal(fetchedGroupUserJson.id, projectUser.id);
    assert.equal(fetchedGroupUserJson.user_type, "user");
    assert.equal(fetchedGroupUserJson.is_service_account, false);

    const userRole = await fetch(`${baseUrl}/v1/organization/users/${projectUser.id}/roles`, {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({ role_id: role.id }),
    });
    assert.equal(userRole.status, 200);
    const userRoleJson = await userRole.json();
    assert.equal(userRoleJson.object, "user.role");
    ...
    assert.equal((await fetchedUserRole.json()).permissions[0], "api.groups.read");

    const groupRole = await fetch(`${baseUrl}/v1/organization/groups/${group.id}/roles`, {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({ role_id: role.id }),
    });
    assert.equal(groupRole.status, 200);
    const groupRoleJson = await groupRole.json();
    assert.equal(groupRoleJson.object, "group.role");
    ...
    assert.equal((await fetchedGroupRole.json()).name, "Bridge Group Operator");

    const missingGroupUser = await fetch(`${baseUrl}/v1/organization/groups/${group.id}/users/user_missing`);
    assert.equal(missingGroupUser.status, 404);
    assert.equal((await missingGroupUser.json()).error.code, "organization_group_user_not_found");

    const missingRoleAssignment = await fetch(`${baseUrl}/v1/organization/users/${projectUser.id}/roles/role_missing`);
    assert.equal(missingRoleAssignment.status, 404);
    assert.equal((await missingRoleAssignment.json()).error.code, "organization_user_role_not_found");

    const deletedUserRole = await fetch(`${baseUrl}/v1/organization/users/${projectUser.id}/roles/${role.id}`, {
      method: "DELETE",
    });
    assert.equal(deletedUserRole.status, 200);
    assert.deepEqual(await deletedUserRole.json(), {
    ...
    assert.equal(deletedRole.status, 200);
    assert.deepEqual(await deletedRole.json(), {
      object: "role.deleted",
      id: role.id,
      deleted: true,
    });
    ...
    assert.deepEqual(await deletedGroup.json(), {
      object: "group.deleted",
      id: group.id,
      deleted: true,
    });

    const missingRole = await fetch(`${baseUrl}/v1/organization/roles/${role.id}`);
    assert.equal(missingRole.status, 404);
    assert.equal((await missingRole.json()).error.code, "organization_role_not_found");

    const missingGroup = await fetch(`${baseUrl}/v1/organization/groups/${group.id}`);
    assert.equal(missingGroup.status, 404);
    assert.equal((await missingGroup.json()).error.code, "organization_group_not_found");
    assert.equal(requests.length, 0);
  });
});
...
test("POST /v1/completions validates seed before provider calls", async () => {
  await withMockProvider(async (_req, res, call) => {
    assert.equal(call.body.seed, 7);
    res.writeHead(200, { "content-type": "application/json" });
    res.end(JSON.stringify({
      id: "chatcmpl_legacy_seed_boundary",
      object: "chat.completion",
      created: 1700000126,
      model: "mock-model",
      choices: [{
        index: 0,
        message: { role: "assistant", content: "legacy seed ok" },
        finish_reason: "stop",
      }],
    }));
  }, async ({ bridgeAddress, requests }) => {
    const baseUrl = `http://127.0.0.1:${bridgeAddress.port}`;
    const invalidCases = [
      { seed: "7", message: "seed must be an integer" },
      { seed: 7.5, message: "seed must be an integer" },
      { seed: [], message: "seed must be an integer" },
      { seed: {}, message: "seed must be an integer" },
      {
        seed: 1e21,
    ...
    for (const invalidCase of invalidCases) {
      const response = await fetch(`${baseUrl}/v1/completions`, {
        ...
        body: JSON.stringify({
          model: "mock-model",
          prompt: "Check legacy seed validation.",
          seed: invalidCase.seed,
        }),
      });
      assert.equal(response.status, 400);
    ...
    const valid = await fetch(`${baseUrl}/v1/completions`, {
      ...
      body: JSON.stringify({
        model: "mock-model",
        prompt: "Check legacy seed boundary.",
        seed: 7,
      }),
    });
    assert.equal(valid.status, 200);
    const json = await valid.json();
    assert.equal(json.choices[0].text, "legacy seed ok");
    assert.equal(requests.length, 1);
  });
});
...
test("POST /v1/completions validates n before provider calls", async () => {
  await withMockProvider(async (_req, res, call) => {
    ...
    res.writeHead(200, { "content-type": "application/json" });
    res.end(JSON.stringify({
      id: "chatcmpl_legacy_n_boundary",
      object: "chat.completion",
      created: 1700000125,
      model: "mock-model",
      choices: [{
        index: 0,
        message: { role: "assistant", content: "legacy n ok" },
        finish_reason: "stop",
      }],
    }));
  }, async ({ bridgeAddress, requests }) => {
    ...
    for (const value of invalidValues) {
      const response = await fetch(`${baseUrl}/v1/completions`, {
        ...
        body: JSON.stringify({
          model: "mock-model",
          prompt: "Check legacy n validation.",
          n: value,
        }),
      });
      assert.equal(response.status, 400);
    ...
    const valid = await fetch(`${baseUrl}/v1/completions`, {
      ...
      body: JSON.stringify({
        model: "mock-model",
        prompt: "Check legacy n boundary.",
        n: 128,
      }),
    });
    assert.equal(valid.status, 200);
    assert.equal((await valid.json()).choices[0].text, "legacy n ok");
    assert.equal(requests.length, 1);
  });
});
...
test("POST /v1/chat/completions validates top_logprobs contract before provider calls", async () => {
  await withMockProvider(async (_req, res, call) => {
    assert.equal(call.body.logprobs, true);
    assert.equal(call.body.top_logprobs, 20);
    res.writeHead(200, { "content-type": "application/json" });
    res.end(JSON.stringify({
      id: "chatcmpl_chat_top_logprobs_boundary",
      object: "chat.completion",
      created: 1700000300,
      model: "mock-model",
      choices: [{
        index: 0,
        message: { role: "assistant", content: "chat logprobs ok" },
        finish_reason: "stop",
      }],
    }));
  }, async ({ bridgeAddress, requests }) => {
    ...
    const invalidRange = await fetch(`${baseUrl}/v1/chat/completions`, {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({
        model: "mock-model",
    ...
    assert.deepEqual(await invalidRange.json(), {
      error: {
        message: "top_logprobs must be an integer between 0 and 20",
        type: "invalid_request_error",
        param: "top_logprobs",
    ...
    for (const logprobs of [undefined, false]) {
      ...
      assert.deepEqual(await response.json(), {
        error: {
          message: "logprobs must be true when top_logprobs is set",
          type: "invalid_request_error",
          param: "logprobs",
          code: "invalid_request_parameter",
        },
...
test("POST /v1/chat/completions validates seed before provider calls", async () => {
  await withMockProvider(async (_req, res, call) => {
    assert.equal(call.body.seed, 123);
    res.writeHead(200, { "content-type": "application/json" });
    res.end(JSON.stringify({
      id: "chatcmpl_chat_seed",
      object: "chat.completion",
      created: 1700000303,
      model: "mock-model",
      choices: [{
        index: 0,
        message: { role: "assistant", content: "chat seed ok" },
        finish_reason: "stop",
      }],
    }));
  }, async ({ bridgeAddress, requests }) => {
    const baseUrl = `http://127.0.0.1:${bridgeAddress.port}`;
    const invalidCases = [
      { seed: "123", message: "seed must be an integer" },
      { seed: 123.5, message: "seed must be an integer" },
      { seed: [], message: "seed must be an integer" },
      { seed: {}, message: "seed must be an integer" },
      {
        seed: -1e21,
...
test("POST /v1/chat/completions validates sampling parameter ranges before provider calls", async () => {
  await withMockProvider(async (_req, res, call) => {
    ...
    res.end(JSON.stringify({
      ...
      choices: [{
        index: 0,
        message: { role: "assistant", content: "chat sampling ok" },
        finish_reason: "stop",
      }],
    }));
  }, async ({ bridgeAddress, requests }) => {
    ...
    assert.equal(valid.status, 200);
    assert.equal((await valid.json()).choices[0].message.content, "chat sampling ok");
    assert.equal(requests.length, 1);
  });
});

test("POST /v1/chat/completions validates stop sequences before provider calls", async () => {
  await withMockProvider(async (_req, res, call) => {
    assert.equal(call.body.stop, "<END>");
    res.writeHead(200, { "content-type": "application/json" });
    res.end(JSON.stringify({
      id: "chatcmpl_chat_stop_boundary",
      object: "chat.completion",
      created: 1700000302,
      model: "mock-model",
      choices: [{
        index: 0,
        message: { role: "assistant", content: "chat stop ok" },
        finish_reason: "stop",
      }],
    }));
  }, async ({ bridgeAddress, requests }) => {
...
test("POST /v1/chat/completions validates n before provider calls", async () => {
  await withMockProvider(async (_req, res, call) => {
    assert.equal(call.body.n, 128);
    res.writeHead(200, { "content-type": "application/json" });
    res.end(JSON.stringify({
      id: "chatcmpl_chat_n_boundary",
      object: "chat.completion",
      created: 1700000304,
      model: "mock-model",
      choices: [{
        index: 0,
        message: { role: "assistant", content: "chat n ok" },
        finish_reason: "stop",
      }],
    }));
  }, async ({ bridgeAddress, requests }) => {
    ...
    assert.equal(valid.status, 200);
    assert.equal((await valid.json()).choices[0].message.content, "chat n ok");
    assert.equal(requests.length, 1);
  });
});

test("POST /v1/chat/completions validates logit_bias before provider calls", async () => {
  await withMockProvider(async (_req, res, call) => {
    assert.deepEqual(call.body.logit_bias, { "42": -100, "99": 100 });
    res.writeHead(200, { "content-type": "application/json" });
    res.end(JSON.stringify({
      id: "chatcmpl_chat_logit_bias_boundary",
      object: "chat.completion",
      created: 1700000303,
      model: "mock-model",
      choices: [{
        index: 0,
        message: { role: "assistant", content: "chat logit bias ok" },
        finish_reason: "stop",
      }],
    }));
  }, async ({ bridgeAddress, requests }) => {
...
test("POST /v1/chat/completions falls back to text markers for direct Chat audio inputs", async () => {
  const audioBase64 = "UklGRg==";
  await withMockProvider(async (_req, res, call) => {
    assert.equal(call.body.store, true);
    assert.deepEqual(call.body.messages.map((message) => message.role), ["user"]);
    assert.equal(typeof call.body.messages[0].content, "string");
    assert.match(call.body.messages[0].content, /Inspect direct Chat audio markers\./);
    assert.match(call.body.messages[0].content, /\[audio:wav:tone\.wav\]/);
    assert.match(call.body.messages[0].content, /spoken transcript/);
    assert.match(call.body.messages[0].content, /\[audio:mp3:clip\.mp3\]/);
    assert.doesNotMatch(call.body.messages[0].content, new RegExp(audioBase64));
    assert.doesNotMatch(call.body.messages[0].content, /input_audio/);

    res.writeHead(200, { "content-type": "application/json" });
    res.end(JSON.stringify({
      id: "chatcmpl_direct_audio_text_fallback",
      object: "chat.completion",
      created: 1700000412,
      model: "mock-model",
      choices: [{
        index: 0,
        message: { role: "assistant", content: "chat-audio-fallback-ok" },
        finish_reason: "stop",
      }],
      usage: { prompt_tokens: 13, completion_tokens: 4, total_tokens: 17 },
    }));
    ...
    const response = await fetch(`${baseUrl}/v1/chat/completions`, {
      ...
      body: JSON.stringify({
        model: "mock-model",
        store: true,
        metadata: { suite: "direct-chat-audio-fallback" },
        messages: [{
          role: "user",
          content: [
            { type: "text", text: "Inspect direct Chat audio markers." },
            {
              type: "input_audio",
              input_audio: {
                data: audioBase64,
    ...
    assert.equal(response.status, 200);
    const json = await response.json();
    assert.equal(json.choices[0].message.content, "chat-audio-fallback-ok");
    assert.equal(json.metadata.compatibility.chat_passthrough.chat_audio_inputs.provider, "text_fallback");
    assert.equal(json.metadata.compatibility.chat_passthrough.chat_audio_inputs.mode, "text");
    assert.equal(json.metadata.compatibility.chat_passthrough.chat_audio_inputs.audio_part_count, 2);
    assert.equal(json.metadata.compatibility.chat_passthrough.chat_audio_inputs.inline_audio_count, 2);
    assert.equal(json.metadata.compatibility.chat_passthrough.chat_audio_inputs.transcript_count, 1);
    assert.equal(requests.length, 1);

    const fetched = await fetch(`${baseUrl}/v1/chat/completions/${json.id}`);
    assert.equal(fetched.status, 200);
    const fetchedJson = await fetched.json();
    assert.equal(fetchedJson.metadata.compatibility.chat_passthrough.chat_audio_inputs.audio_part_count, 2);
...
test("local Batch API executes direct Images variation JSONL", async () => {
  const sourcePng = "iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR42mP8/x8AAwMCAO+/p9sAAAAASUVORK5CYII=";
  await withMockProvider(async () => {
    assert.fail("chat provider should not be called for placeholder direct image variation batches");
  }, async ({ bridgeAddress, requests }) => {
    const baseUrl = `http://127.0.0.1:${bridgeAddress.port}`;
    const jsonl = `${JSON.stringify({
      custom_id: "direct-image-variation-ok",
      method: "POST",
      url: "/v1/images/variations",
      body: {
        model: "dall-e-2",
        image: { image_url: `data:image/png;base64,${sourcePng}`, filename: "batch-source.png" },
        n: 2,
        size: "1024x1024",
        response_format: "b64_json",
      },
    ...
    const fileResponse = await fetch(`${baseUrl}/v1/files`, {
      ...
      headers: { "content-type": "application/json" },
      body: JSON.stringify({
        filename: "images-variation-batch.jsonl",
        purpose: "batch",
        content_base64: Buffer.from(jsonl, "utf8").toString("base64"),
        mime_type: "application/jsonl",
      }),
    ...
    const created = await fetch(`${baseUrl}/v1/batches`, {
      ...
      body: JSON.stringify({
        ...
        completion_window: "24h",
        metadata: { suite: "batch-images-variations" },
      }),
    });
    assert.equal(created.status, 200);
    const batch = await created.json();
    assert.equal(batch.object, "batch");
    assert.equal(batch.status, "completed");
    assert.equal(batch.endpoint, "/v1/images/variations");
    assert.equal(batch.request_counts.total, 1);
    assert.equal(batch.request_counts.completed, 1);
    assert.equal(batch.request_counts.failed, 0);
    assert.ok(batch.output_file_id);
    assert.equal(batch.error_file_id, null);
    assert.equal(batch.metadata.compatibility.supported_endpoints.includes("/v1/images/variations"), true);
    assert.equal(requests.length, 0);
    ...
    const outputLines = (await outputResponse.text()).trim().split(/\n/).map((line) => JSON.parse(line));
    assert.equal(outputLines.length, 1);
    assert.equal(outputLines[0].custom_id, "direct-image-variation-ok");
    assert.equal(outputLines[0].response.status_code, 200);
    const body = outputLines[0].response.body;
    assert.equal(typeof body.created, "number");
    assert.equal(body.data.length, 2);
    assert.deepEqual(Buffer.from(body.data[0].b64_json, "base64").subarray(0, 8), Buffer.from("89504e470d0a1a0a", "hex"));
    assert.deepEqual(Buffer.from(body.data[1].b64_json, "base64").subarray(0, 8), Buffer.from("89504e470d0a1a0a", "hex"));
    assert.match(body.data[0].revised_prompt, /variation of the supplied image/);
  }, {
    imageGenerationPlaceholderSize: 16,
  });
});
...
test("loadConfig filters provider-specific Chat fields for DeepSeek providers by default", () => {
  ...
  try {
    ...
    process.env.CODEXCOMPAT_CHAT_AUDIO_INPUT_MODE = "audio";
    assert.equal(loadConfig({ providerBaseUrl: "https://api.deepseek.com" }).chatAudioInputMode, "audio");
