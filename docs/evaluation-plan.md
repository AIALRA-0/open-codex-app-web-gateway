...
- Local project-role short-path coverage exercises the official
  `/v1/projects/{project_id}/roles`,
  `/v1/projects/{project_id}/users/{user_id}/roles`, and
  `/v1/projects/{project_id}/groups/{group_id}/roles` families against the
  local admin store: project-scoped role create/list/get/update/delete,
  direct project-user and project-group assignment lifecycle, missing user/role
  errors, archived-project rejection, assignment cleanup after role deletion,
  and zero upstream Chat provider calls.
...
- Responses `truncation:"auto"` drops oldest local `conversation` /
  ...
  `context_length_exceeded` error before provider calls. Responses
  `truncation` values must also validate as `auto`, `disabled`, or null on
  create, `/input_tokens`, and the local `/compact` compatibility path before
  replay, local context truncation, or provider calls.
...
- Responses `context_management` request compatibility records the official
  ...
  prove it rejects empty arrays, validates `type:"compaction"` and numeric
  `compact_threshold`, is not forwarded to Chat Completions providers,
  compatibility metadata exposes only entry counts/types/threshold presence, and
  caller-provided threshold values do not leak into compatibility metadata. The
  older local `context` alias remains covered as a non-forwarded compatibility
  boundary with object-key-only
  metadata.
...
- Local Batch API accepts `purpose:"batch"` JSONL Files, validates per-line
  non-empty string and unique `custom_id` values, validates non-empty string
  `method` and `url` values, rejects non-`POST` methods and endpoint
  mismatches, rejects mixed string `body.model` values within a single input
  file, executes supported endpoints including Responses create,
  `/v1/responses/input_tokens`,
  `/v1/responses/compact`, Responses `image_generation`, direct
  `/v1/audio/transcriptions`, direct `/v1/audio/translations`, direct
  `/v1/images/generations`, JSON-form direct `/v1/images/edits`, JSON-form
  direct `/v1/images/variations`, and direct `/v1/videos` requests, enforces the
  aggregate embeddings input-count cap before execution, exposes OpenAI-style
  Batch objects, writes output/error JSONL Files that can be read through
  `/v1/files/{file_id}/content`, and lazily cleans up expired output/error Files
  on Batch list/get/cancel when `output_expires_after` has elapsed.
...
- Hosted-tool emulation returns auditable shell call/output items and downloadable artifacts for shell/code-interpreter requests, and local Containers validate create-time `memory_limit` enum values, `network_policy` disabled/allowlist shapes with redacted `domain_secrets`, and `expires_after:{anchor:"last_active_at",minutes:<positive integer>}`, honor that expiration policy by lazily marking elapsed containers expired, prune their workspaces, reject container file access, block stale Responses shell/code-interpreter reuse with `container_expired`, and reject missing explicit `container_reference` IDs with `container_not_found` before any upstream provider call.
