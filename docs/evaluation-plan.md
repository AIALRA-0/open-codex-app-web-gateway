...
- Local Batch API accepts `purpose:"batch"` JSONL Files, executes supported endpoints including Responses create, `/v1/responses/input_tokens`, `/v1/responses/compact`, Responses `image_generation`, direct `/v1/audio/transcriptions`, direct `/v1/audio/translations`, direct `/v1/images/generations`, JSON-form direct `/v1/images/edits`, JSON-form direct `/v1/images/variations`, and direct `/v1/videos` requests, enforces the aggregate embeddings input-count cap before execution, exposes OpenAI-style Batch objects, writes output/error JSONL Files that can be read through `/v1/files/{file_id}/content`, and lazily cleans up expired output/error Files on Batch list/get/cancel when `output_expires_after` has elapsed.
