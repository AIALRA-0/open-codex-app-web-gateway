...
- Treat audio-capable Chat providers as provider-specific protocol coverage:
  unit tests must preserve `message.audio` and streaming `delta.audio` into
  Responses output, metadata, and replay, while live DeepSeek runs only verify
  that text-only providers remain stable when audio request fields are filtered.
