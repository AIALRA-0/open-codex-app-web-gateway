...
- Local Batch API accepts `purpose:"batch"` JSONL Files, validates per-line
  non-empty string and unique `custom_id` values, rejects mixed string
  `body.model` values within a single input file, executes supported endpoints
  including Responses create, `/v1/responses/input_tokens`,
