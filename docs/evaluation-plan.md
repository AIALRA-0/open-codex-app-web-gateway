...
- Responses and direct Chat sampling requests validate official `temperature` and `top_p` numeric bounds locally before provider calls, while valid boundary values still pass through to upstream Chat providers unchanged.
