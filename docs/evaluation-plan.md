...
- Hosted-tool emulation returns auditable shell call/output items and downloadable artifacts for shell/code-interpreter requests, and local Containers validate create-time `expires_after:{anchor:"last_active_at",minutes:<positive integer>}`, honor that policy by lazily marking elapsed containers expired, prune their workspaces, reject container file access, block stale Responses shell/code-interpreter reuse with `container_expired`, and reject missing explicit `container_reference` IDs with `container_not_found` before any upstream provider call.
