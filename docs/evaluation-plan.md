...
- Stale in-progress background responses left by a bridge restart either resume
  from a persisted `provider_pending` snapshot or reconcile to explicit failed
  terminal records when no safe resume snapshot exists.
