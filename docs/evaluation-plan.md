...
- Treat request-based Audio APIs as bridge-owned protocol coverage: unit tests
  ...
  and translation, transcription SSE, and local Batch JSONL transcription /
  translation; live runs verify deterministic placeholder stability and route
  shape.
...
- Direct Audio API evaluation covers `/v1/audio/speech` placeholder bytes,
  ...
  paths, `/v1/audio/translations` multipart and JSON/base64 paths, and local
  Batch JSONL execution over audio transcription and translation requests.
