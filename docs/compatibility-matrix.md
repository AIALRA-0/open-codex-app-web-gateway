...
| `POST /v1/responses/input_tokens` | Implemented via upstream usage probe | Validates official token-count request fields before provider calls, including `truncation`, `previous_response_id`/`conversation`, `parallel_tool_calls`, `text.format`, and `reasoning.effort`; translates the request, local `conversation` state, local input files/images, and tool schemas to Chat Completions; validates the official `style` preset limit of 64 characters and maps valid `style` values to a Chat-visible compatibility instruction for the prompt-token probe; forces non-streaming `max_tokens:1`, strips streaming options, disables upstream storage, and returns `usage.prompt_tokens` as `input_tokens` without appending Conversation items |
