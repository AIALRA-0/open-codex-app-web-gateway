...
| `GET /v1/chat/completions` | Implemented for local `store:true` records | Lists locally stored upstream Chat completion objects with `model`, `metadata[key]`, `limit`, `after`, and `order` filters; legacy local records are projected through the current stored Chat completion normalizer before filtering and pagination; `order` is validated against the official `asc` / `desc` enum and defaults to `asc`, and `limit` must be a positive integer query value |
