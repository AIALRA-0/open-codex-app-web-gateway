...
| `temperature`, `top_p`, penalties, `seed`, `user` | same-name fields | Provider-dependent after OpenAI request validation. The bridge validates user-supplied Responses and direct Chat `temperature` as a number from 0 through 2 and `top_p` as a number from 0 through 1 before provider calls; valid values are forwarded to upstream Chat providers unchanged. Sampling penalties remain provider-dependent Chat-native passthrough |
