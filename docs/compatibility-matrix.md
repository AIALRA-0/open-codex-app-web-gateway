...
`audio`, `moderation`, `prediction`, `parallel_tool_calls`,
`prompt_cache_retention`, and legacy `functions` / `function_call` are filtered
instead of being sent to the provider. DeepSeek-supported Chat log-probability controls such as `logprobs`
