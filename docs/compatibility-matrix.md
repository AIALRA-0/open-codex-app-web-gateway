...
| `user_id`, `safety_identifier`, `prompt_cache_key`, `prompt_cache_retention`, `user` | DeepSeek `user_id` plus prompt-cache passthrough/filtering | DeepSeek-specific compatibility after OpenAI request validation. Direct Chat and Responses Chat-native aliases validate `user` and `prompt_cache_key` as strings, `safety_identifier` as a string up to the official 64-character limit, and `prompt_cache_retention` as `in_memory` or `24h`; current official OpenAPI/docs do not publish a maximum length for `prompt_cache_key`, so the bridge no longer applies the older local 64-character cap. DeepSeek `user_id` is direct when already `[A-Za-z0-9_-]`, otherwise stable SHA-256 normalized. Local organization usage dimensions use `user_id`, then `safety_identifier`, then `user`, then `prompt_cache_key` when no explicit usage user id exists |
