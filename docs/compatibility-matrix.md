...
| `POST /v1/threads/{thread_id}/runs` | Implemented via upstream Chat plus local hosted-tool adapters | Creates a local run by calling the configured Chat provider; rejects with `thread_locked` while another non-terminal local run is active on the thread; local Assistants `file_search` and `code_interpreter` tools run first and persist `tool_calls` Run Steps; function tools complete with one assistant message and `message_creation` step or enter `requires_action`; `stream:true` relays Chat text/refusal deltas as `thread.message.delta` and Chat tool-call argument deltas as `thread.run.step.delta` |
