...
| `temperature`, `top_p`, penalties, `seed`, `user` | same-name fields | Provider-dependent after OpenAI request validation. The bridge validates user-supplied Responses and direct Chat `temperature` as a number from 0 through 2, `top_p` as a number from 0 through 1, and Chat-native `frequency_penalty` / `presence_penalty` aliases as numbers from -2 through 2 before provider calls; valid values are forwarded to upstream Chat providers unchanged |
