...
| `temperature`, `top_p`, penalties, `stop`, `seed`, `user` | same-name fields | Provider-dependent after OpenAI request validation. The bridge validates user-supplied Responses and direct Chat `temperature` as a number from 0 through 2, `top_p` as a number from 0 through 1, Chat-native `frequency_penalty` / `presence_penalty` aliases as numbers from -2 through 2, Chat-native `stop` as a string or an array of up to 4 strings, Chat-native `seed` as an integer in the official `-9223372036854776000` through `9223372036854776000` range, and `user` as a string before provider calls; valid values are forwarded to upstream Chat providers unchanged |
...
| legacy Chat `functions` / `function_call` | modern Chat `tools` / `tool_choice` | Provider-aware compatibility for deprecated Chat SDK callers. When Chat-native fields are not forwarded, legacy function definitions are converted to function tools and compatible `function_call` choices are converted to `tool_choice`; the bridge records `metadata.compatibility.legacy_functions` on Responses translation and `metadata.compatibility.chat_passthrough.legacy_functions` on direct Chat requests |
...
| `temperature`, `top_p`, `frequency_penalty`, `presence_penalty`, `stop`, `seed`, `n` | same-name Chat fields | Direct/provider-dependent after request validation; legacy Completions `seed` must be an integer in the official 64-bit-compatible range and `n` must be an integer from 1 through 128 before any upstream Chat request |
...
`audio`, `moderation`, `prediction`, and `prompt_cache_retention` are filtered
instead of being sent to the provider. `parallel_tool_calls` must be a boolean
...
`tools` / `tool_choice` when possible and recorded under
