...
| `function_call_output` item | `role:"tool"` message with `tool_call_id` | Direct after pre-provider schema validation. Responses create, `/input_tokens`, and `/compact` require `call_id` to be a non-empty string no longer than 64 characters and `output` to be a string or input text/image/file content array before the tool output is replayed to Chat |
