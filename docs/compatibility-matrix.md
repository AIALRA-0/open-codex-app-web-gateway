...
| string `input` | `messages: [{role:"user"}]` | Direct. `/v1/responses/input_tokens` and `/v1/responses/compact` enforce the current official 10,485,760-character limit for top-level string `input` before provider calls; `/v1/responses` top-level string `input` remains unconstrained locally because the current public `InputParam` schema does not publish that top-level `maxLength` |
...
| `input_file` / compatible Chat `file` parts | local extraction context plus explicit text marker | Emulated for local Files API `file_id`, completed Uploads API files, inline base64 `file_data`, and HTTP(S) `file_url` when text can be extracted; Responses create, `/input_tokens`, and `/compact` validate official `input_file.detail` as `low` or `high`, require known source fields such as `file_id`, `filename`, `file_data`, and `file_url` plus bridge aliases to be strings when present before provider calls, enforce the official 73,400,320-character `file_data` limit for official `file_data` and compatible inline file-data aliases, and reject malformed official `file_url` URI strings. Responses POST JSON reads use an 83,886,080-byte local body cap so official maximum-size `file_data` payloads can reach schema validation instead of the bridge's default 16 MiB reader limit. PDFs use local Poppler `pdftotext` when enabled and can fall back to bounded local `pdftoppm` + `tesseract` OCR when the text layer is empty; `.docx`, `.xlsx`, and `.pptx` OOXML files are extracted locally from ZIP/XML content. Direct Chat passthrough uses `CODEXCOMPAT_CHAT_FILE_INPUT_MODE`: file-capable providers may receive native file parts, while text-only providers such as DeepSeek receive safe `[file:...]` markers plus extracted local context and record `metadata.compatibility.chat_passthrough.chat_file_inputs` / `local_input_files` |
| prior `message` output item | assistant chat message | Direct after pre-provider schema validation. Assistant replay messages accept string content or output content parts such as `output_text` and `refusal`, validate `output_text.text`, official annotation item shapes (`file_citation`, `url_citation`, `container_file_citation`, `file_path`), token logprob item shapes (`token`, numeric `logprob`, integer `bytes`, and nested `top_logprobs`), refusal text, optional `status`, and Codex `phase`, and map `summary_text` / `reasoning_text` text plus `computer_screenshot` image evidence into Chat-compatible content |
| `function_call` item | assistant `tool_calls[]` | Direct after pre-provider schema validation. Responses create, `/input_tokens`, and `/compact` require `call_id` to be a non-empty string no longer than the official 64-character limit, require string `name`, JSON-string `arguments`, optional string/null `namespace`, and returned `status` values of `in_progress`, `completed`, or `incomplete` before the item is replayed as a Chat function `tool_calls[]` entry |
| `function_call_output` item | `role:"tool"` message with `tool_call_id` | Direct after pre-provider schema validation. Responses create, `/input_tokens`, and `/compact` require `call_id` to be a non-empty string no longer than 64 characters and `output` to be a string or input text/image/file content array before the tool output is replayed to Chat. Official string outputs are capped at 10,485,760 characters, and array content parts keep their own text/image/file source limits |
| `item_reference` input item | bounded `system` context marker | Emulated locally after pre-provider schema validation. Responses create, `/input_tokens`, and `/compact` accept explicit `type:"item_reference"` items, official id-only references such as `{id:"msg_..."}`, and nullable-type references such as `{type:null,id:"msg_..."}` when no message/content/tool-call envelope fields are present. All reference forms require a non-empty `id`, normalize stored id-only/null-type records back to `type:"item_reference"`, and replay the reference as a stable Chat-visible context marker instead of raw JSON fallback. Top-level input items with an explicit unsupported string `type` or a non-string/non-null `type` fail locally before any provider call, while nested message content parts continue to use their own official content-part validation. Chat Completions has no native item-reference primitive, so full dereference still depends on separately replayed local `previous_response_id` / `conversation` context |
| `custom_tool_call` item | assistant Chat custom `tool_calls[]`, or assistant text replay for function-only providers | Provider-dependent after pre-provider schema validation. `call_id` must be a non-empty string no longer than 64 characters. When `CODEXCOMPAT_FORWARD_CHAT_CUSTOM_TOOLS=true`, the bridge replays official Responses `custom_tool_call` items as Chat `tool_calls:[{type:"custom",custom:{name,input}}]`; DeepSeek/function-only deployments use a bounded assistant text replay that preserves `call_id`, `namespace`, `name`, and `input` without sending unsupported native custom tool-call fields upstream |
| `custom_tool_call_output` item | Chat `role:"tool"` custom output, or user text replay for function-only providers | Provider-dependent. Official `custom_tool_call_output` and `function_call_output` input items are validated before provider calls: `call_id` must be non-empty and at most 64 characters, `output` must be a string or input text/image/file content array, and returned `status` values must be `in_progress`, `completed`, or `incomplete`. Function string outputs use the official 10,485,760-character limit; custom string outputs follow the current official schema, which does not publish a string maxLength, while custom output arrays retain the shared content-part limits. Custom outputs are replayed as native tool messages only for providers with custom-tool forwarding enabled; DeepSeek/function-only deployments receive explicit text context instead |
| `computer_call_output` item | Chat-visible computer evidence | Emulated locally after pre-provider schema validation. Returned computer-use output items require `call_id` to be a non-empty string no longer than 64 characters, object `output`, official `status` values when present, valid screenshot image detail, optional `image_url` string/object or `file_id`, and array/null `acknowledged_safety_checks` with string ids. Valid outputs are translated into bounded readable Chat context and stored input-item projections continue to hide `output.image_url` unless `include:["computer_call_output.output.image_url"]` is requested |
| official hosted/local tool result items | bounded `system` context messages | Emulated for Chat-only providers after local input schema validation. Prior `file_search_call`, `web_search_call`, `image_generation_call`, `code_interpreter_call`, `computer_call`, `shell_call`, `shell_call_output`, `local_shell_call`, `local_shell_call_output`, `apply_patch_call`, `apply_patch_call_output`, `mcp_list_tools`, `mcp_call`, `mcp_approval_request`, and `mcp_approval_response` input items validate known status enums, required file/web search ids and status values, official web-search action shapes, required code-interpreter `id`/`status`/`container_id`/`code`/`outputs`, shell/apply-patch/computer/tool-search `call_id` strings against the official 64-character boundary when present, shell replay `action.commands` / integer limits / local or container-reference environments / stdout-stderr-outcome chunks, optional apply-patch ids, required apply-patch statuses, official apply-patch operation types (`create_file`, `delete_file`, `update_file`), required non-empty operation paths, required create/update diffs capped at 10,485,760 characters, official 10,485,760-character caps for shell stdout/stderr chunks and apply-patch output text, string/object/array field shapes, MCP required replay ids, MCP list-tools `error`, required tool `name`/`input_schema`, tool `annotations`, and MCP approval booleans before provider calls, then translate into readable "Prior Responses tool context" messages. The serializer preserves ids, call ids, status, key results, logs, command output, MCP approval/call fields, and bounded extras while summarizing image-generation base64 as `base64_image(<chars> chars)` instead of injecting raw payloads. Tool-search protocol items keep their specialized loading path, and MCP protocol items are skipped when the local MCP adapter is already importing them |
...
| `tool_search_call` / `tool_search_output` input items | local loaded-tool context plus Chat function tools | Emulated locally after pre-provider schema validation. The bridge skips raw Chat text fallback for tool-search protocol items, validates optional `call_id` as null or a non-empty string no longer than 64 characters, validates `tool_search_call` status/execution/arguments and `tool_search_output` status/execution/tool definitions before provider calls, loads `tool_search_output.tools` into Chat function definitions even when the next request does not repeat `tools:[{type:"tool_search"}]`, and records `metadata.compatibility.local_tool_search` |
...
| `text.format.type=text` / `text.verbosity` | omitted/default, Chat `response_format:{type:"text"}`, or Chat `verbosity` | Direct after OpenAI request validation. Responses create and `/input_tokens` `text.format`, plus direct Chat `response_format`, must be objects whose `type` is one of `text`, `json_object`, or `json_schema` before provider calls. Official Responses `text.verbosity` is validated as `low`, `medium`, `high`, or null; when top-level `verbosity` is absent it maps to Chat `verbosity` for capable providers, or to the existing DeepSeek-compatible prompt instruction path when native Chat fields are filtered |
...
| `GET /v1/chat/completions` | Implemented for local `store:true` records | Lists locally stored upstream Chat completion objects with only the official `model`, `metadata[key]`, `limit`, `after`, and `order` filters; unsupported query parameters such as `before` are rejected before listing. Legacy local records are projected through the current stored Chat completion normalizer before filtering and pagination; `order` is validated against the official `asc` / `desc` enum and defaults to `asc`, `limit` must be a positive integer query value, scalar `model` / `after` query parameters may appear only once, and `metadata[key]` filters follow the same official 16-pair, 64-character key, 512-character value limits used for stored Chat metadata |
| `GET /v1/chat/completions/{completion_id}` | Implemented for local `store:true` records | Rejects unsupported query parameters before local store lookup, then returns a locally stored upstream Chat completion object projected through the current stored Chat completion normalizer, so records created before newer shape fixes expose the same official fields as new records |
| `POST /v1/chat/completions/{completion_id}` | Implemented for local `store:true` records | Rejects unsupported query parameters before JSON body parsing, then updates only the stored completion `metadata` field, matching the current OpenAI API restriction for stored Chat Completions; accepts nullable official Metadata by clearing local metadata to `{}`; returns the updated record through the same stored Chat projection used by retrieve/list |
| `DELETE /v1/chat/completions/{completion_id}` | Implemented for local `store:true` records | Rejects unsupported query parameters before local deletion, then deletes a locally stored Chat completion and returns `object:"chat.completion.deleted"` |
| `GET /v1/chat/completions/{completion_id}/messages` | Implemented for local `store:true` records | Returns request messages plus assistant choice messages with only the official `limit`, `after`, and `order` pagination parameters; unsupported query parameters such as `before` are rejected before message projection. Message records include official `id`, `role`, `content`, `name`, and `content_parts` fields, and keep local `direction` for replay/debugging. Legacy local message records are projected through the current message normalizer before pagination. Assistant/output message records expose missing `refusal:null`, `annotations:[]`, `tool_calls:null`, and `function_call:null`, while input messages are not given assistant-only fields. `content_parts` is populated only when the stored Chat content-part array contains official `text` / `image_url` parts and is `null` for string content or bridge extension parts such as audio/file inputs. `order` is validated against the official `asc` / `desc` enum and defaults to `asc`, `limit` must be a positive integer query value, and scalar `after` may appear only once |
...
| `GET /v1/fine_tuning/jobs` | Implemented locally | Lists local jobs with official `limit` and `after` pagination plus `metadata[k]=v` / `metadata=null` filters only; repeated official scalar query values return 400, invalid metadata filter syntax returns 400, unsupported query parameters such as `before` and `order` return 400 before reading local state, and default order is newest-first |
...
| `GET /v1/fine_tuning/jobs/{fine_tuning_job_id}/events` | Implemented locally | Lists OpenAI-style `fine_tuning.job.event` records for creation, simulated run, completion, and lifecycle actions with official `limit` and `after` pagination only; repeated official scalar query values return 400, unsupported query parameters such as `before` and `order` return 400 before reading local state, and default order is newest-first |
| `GET /v1/fine_tuning/jobs/{fine_tuning_job_id}/checkpoints` | Implemented locally | Lists a synthetic `fine_tuning.job.checkpoint` with `fine_tuned_model_checkpoint`, `metrics`, `fine_tuning_job_id`, and `step_number` using official `limit` and `after` pagination only; repeated official scalar query values return 400, unsupported query parameters such as `before` and `order` return 400 before reading local state, and default order is newest-first |
| `GET /v1/fine_tuning/checkpoints/{fine_tuned_model_checkpoint}/permissions` | Implemented locally | Lists local `checkpoint.permission` records with official `project_id`, `limit`, `after`, and `order` controls only; `order` is validated against the official `ascending` / `descending` enum with default `descending`, `limit` must be a positive integer query value with default 10, scalar `project_id` / `after` query parameters may appear only once, and unsupported query parameters such as `before` return 400 before reading local state |
...
| Endpoint | Status | Notes |
| --- | --- | --- |
| `GET /v1/organization/users` | Implemented locally | Lists local `organization.user` records with only the official `limit`, `after`, and `emails` query parameters plus compatibility `emails[]` filtering; nested local project memberships and OpenAI-style pagination are preserved; repeated scalar values return 400, `limit` is enforced from 1 through 100 with official default 20, and unsupported query parameters such as `order` / `before` return 400 before listing |
...
| `GET /v1/organization/invites` | Implemented locally | Lists local `organization.invite` records with only the official `limit` and `after` query parameters plus OpenAI-style pagination; repeated scalar values return 400, `limit` is enforced from 1 through 100 with official default 20, and unsupported query parameters such as `order` / `before` return 400 before listing |
...
| `GET /v1/organization/admin_api_keys` | Implemented locally | Lists local redacted `organization.admin_api_key` records with only the official `limit`, `after`, and `order` query parameters plus OpenAI-style pagination; persisted records never include a usable `value`; repeated scalar values return 400, `limit` is enforced from 1 through 100 with official default 20, official `order:"asc"|"desc"` is supported, and unsupported query parameters such as `before` return 400 before listing |
...
| `GET /v1/organization/certificates` | Implemented locally | Lists local uploaded certificate records as `organization.certificate` with organization-scope `active` flags and only the official `limit`, `after`, and `order` query parameters plus OpenAI-style cursor pagination; PEM content is omitted; repeated scalar values return 400, `limit` is enforced from 1 through 100 with official default 20, official `order:"asc"|"desc"` is supported with default `desc`, and unsupported query parameters such as `before` return 400 before listing |
...
| `GET /v1/organization/audit_logs` | Implemented locally | Lists local Organization admin lifecycle events generated by this compatibility store with only official filtering/pagination parameters: `effective_at[gt/gte/lt/lte]`, `project_ids[]`, `event_types[]`, `actor_ids[]`, `actor_emails[]`, `resource_ids[]`, `tenant_only`, `after`, `before`, and `limit`, while preserving local `effective_at.gte` / `effective_at_gte` aliases; validates `limit` from 1 through 100 with default 20, single `after`/`before`, boolean `tenant_only`, and integer `effective_at` range values; unsupported query parameters such as `order` return 400 before listing; returns OpenAI list fields `first_id`, `last_id`, and `has_more`. It is not a hosted OpenAI security audit export |
...
| `GET /v1/organization/groups` | Implemented locally | Lists local `group` records with OpenAI-style `next` cursor pagination; validates official `limit` from 0 through 1000 with default 100, `after`, and `order:"asc"|"desc"`, rejects repeated scalar values, and rejects unsupported query parameters such as `before` before listing |
...
| `GET /v1/organization/users/{user_id}/roles` | Implemented locally | Lists local organization roles directly assigned to a local user, including assignment-source metadata, with OpenAI SDK `NextCursorPage` semantics; validates `limit` from 0 through 1000 with local default 20, `after`, and `order:"asc"|"desc"`, rejects repeated scalar values, and rejects unsupported query parameters such as `before` before listing |
...
| `GET /v1/organization/groups/{group_id}/roles` | Implemented locally | Lists local organization roles assigned to a local group, including assignment-source metadata, with OpenAI SDK `NextCursorPage` semantics; validates `limit` from 0 through 1000 with local default 20, `after`, and `order:"asc"|"desc"`, rejects repeated scalar values, and rejects unsupported query parameters such as `before` before listing |
...
| `GET /v1/projects/{project_id}/users/{user_id}/roles` | Implemented locally | Lists local project roles directly assigned to an existing project user, including assignment-source metadata and `project_id`, with OpenAI SDK `NextCursorPage` semantics; validates `limit` from 0 through 1000 with local default 20, `after`, and `order:"asc"|"desc"`, rejects repeated scalar values, and rejects unsupported query parameters such as `before` before listing |
...
| `GET /v1/projects/{project_id}/groups/{group_id}/roles` | Implemented locally | Lists local project roles directly assigned to an existing project group access record, including assignment-source metadata and `project_id`, with OpenAI SDK `NextCursorPage` semantics; validates `limit` from 0 through 1000 with local default 20, `after`, and `order:"asc"|"desc"`, rejects repeated scalar values, and rejects unsupported query parameters such as `before` before listing |
...
| `GET /v1/organization/projects` | Implemented locally | Lists local `organization.project` records with only the official `limit`, `after`, and `include_archived` query parameters plus OpenAI-style `object:"list"`, `first_id`, `last_id`, and `has_more`; repeated scalar values return 400, `limit` is enforced from 1 through 100 with official default 20, archived projects are excluded unless `include_archived=true`, and unsupported query parameters such as `order` / `before` return 400 before listing |
...
| `GET /v1/organization/projects/{project_id}/users` | Implemented locally | Lists local `organization.project.user` records for active projects with only the official `limit` and `after` query parameters; repeated scalar values return 400, `limit` is enforced from 1 through 100 with official default 20, and unsupported query parameters such as `order` / `before` return 400 before listing |
...
| `GET /v1/organization/projects/{project_id}/groups` | Implemented locally | Lists local `project.group` access records for active projects with OpenAI SDK `NextCursorPage` semantics using `group_id` as the cursor; validates official `limit` from 0 through 100 with default 20, `after`, and `order:"asc"|"desc"`, rejects repeated scalar values and unsupported query parameters such as `before`, and archived projects return `400 project_archived` |
...
| `GET /v1/organization/projects/{project_id}/certificates` | Implemented locally | Lists local project certificate records for active projects as `organization.project.certificate`, including project-scope `active` flags and no PEM content, with only the official certificate-list `limit`, `after`, and `order` query parameters plus OpenAI list fields `first_id`, `last_id`, and `has_more`; repeated scalar values return 400, `limit` is enforced from 1 through 100 with official default 20, official `order:"asc"|"desc"` is supported with default `desc`, unsupported query parameters such as `before` return 400 before listing, and archived projects return `400 project_archived` |
...
| `GET /v1/organization/projects/{project_id}/api_keys` | Implemented locally | Lists local redacted `organization.project.api_key` records with only the official `limit` and `after` query parameters; persisted records never include a secret `value`; repeated scalar values return 400, `limit` is enforced from 1 through 100 with official default 20, and unsupported query parameters such as `order` / `before` return 400 before listing |
...
| `GET /v1/organization/projects/{project_id}/service_accounts` | Implemented locally | Lists local `organization.project.service_account` records for active projects with only the official `limit` and `after` query parameters; repeated scalar values return 400, `limit` is enforced from 1 through 100 with official default 20, and unsupported query parameters such as `order` / `before` return 400 before listing |
...
| `GET /v1/organization/projects/{project_id}/rate_limits` | Implemented locally | Lazily seeds and lists local `project.rate_limit` records with model, requests-per-minute, tokens-per-minute, and relevant optional limits such as requests/day, images/min, audio MB/min, and batch input tokens/day; accepts only the official `limit`, `after`, and `before` query parameters, repeated scalar values return 400, `limit` is enforced from 1 through 100 with official default 100, and unsupported query parameters such as `order` return 400 before listing |
...
| `GET /v1/models/{model}` | Implemented | Accepts the official no-query retrieve shape; unsupported query parameters return 400 before upstream proxying or local catalog fallback. Proxies upstream single-model retrieval when supported; otherwise searches upstream model list, then falls back to local bridge model objects for the configured default, embeddings, or moderations model IDs |
| `DELETE /v1/models/{model}` | Implemented for upstream-compatible fine-tuned models | Accepts the official no-query delete shape; unsupported query parameters return 400 before any upstream delete attempt. Proxies upstream model deletion and normalizes successful `{deleted:true}` responses to OpenAI-style `{id,object:"model",deleted:true}`; when no upstream deletion is available, returns local `404 model_not_found` instead of deleting local bridge catalog models |
...
Each local JSONL line must use non-empty string `custom_id`, `method`, and
`url` values; `method` must be `POST`, and custom IDs must be unique within the
input file so output/error lines remain reliably matchable to inputs. When
JSONL request bodies include a string `model`, the input file must use one
model value throughout, matching the OpenAI Batch guide single-model boundary.
...
| Endpoint | Status | Notes |
| --- | --- | --- |
| `POST /v1/batches` | Implemented locally | Rejects unsupported query parameters before JSON parsing, file lookup, provider calls, or batch record creation; requires string `input_file_id`, `endpoint`, and `completion_window:"24h"`; validates user-supplied `metadata` against the official string Metadata limits and validates `output_expires_after:{anchor:"created_at",seconds:3600..2592000}` before file lookup or provider calls; validates `purpose:"batch"` input files; executes JSONL lines synchronously through the existing local endpoint handlers, rejecting per-line missing/non-string/blank/duplicate `custom_id` values, missing/non-string/blank `method` or `url` values, non-`POST` methods, endpoint mismatches, and mixed string `body.model` values into the Batch error file, and preserving each line URL's query string so per-endpoint query validation can reject unsupported line-level parameters instead of silently dropping them; writes successful lines to a `purpose:"batch_output"` File and failed lines to a `purpose:"batch_error"` File with local output-expiration metadata when requested |
...
Embeddings batches additionally enforce an aggregate input-count cap before
executing any line. The default `CODEXCOMPAT_BATCH_MAX_EMBEDDING_INPUTS=50000`
matches the current OpenAI Batch embeddings limit; deployments can lower it to
protect disk and CPU on constrained hosts. Token-id arrays count as one
embedding input, while arrays of strings or token arrays count per element.
...
| `GET /v1/files` | Implemented | Lists local files with only the official `purpose`, `limit`, `after`, and `order` pagination/filter parameters; unsupported query parameters such as `before` return 400 before listing. Repeated scalar query values return 400, and `limit` is validated from 1 through 10000 |
...
- Responses `input_image.image_url` strings, compatible `image_url.url`
  objects, and inline image-source aliases are validated against the official
  20,971,520-character image URL/data URL limit before provider calls;
...
Responses create, `/v1/responses/input_tokens`, and `/v1/responses/compact`
read JSON bodies with an 83,886,080-byte local cap for official multimodal
inputs, then validate `input_file.file_data` and compatible inline file-data
aliases against the official 73,400,320-character limit before provider calls.
Other JSON endpoints keep the bridge's smaller default body reader.
...
| `POST /v1/audio/voice_consents` | Implemented locally | Accepts the official no-query create shape before body parsing; accepts official-style multipart `name`, `language`, and `recording` plus JSON/base64 compatibility; stores local metadata only with `cons_*` ids, file byte counts, content type, format, and sha256 |
| `GET /v1/audio/voice_consents` | Implemented locally | Lists local consent recording metadata with only the official `after` and `limit` pagination parameters; unsupported query parameters such as `before` and `order` return 400 before listing. `limit` is validated from 1 through 100 with default 20, and scalar `after` may appear only once |
| `GET /v1/audio/voice_consents/{consent_id}` | Implemented locally | Rejects unsupported query parameters before reading local state, then retrieves local consent metadata by id |
| `DELETE /v1/audio/voice_consents/{consent_id}` | Local cleanup extension | Rejects unsupported query parameters before deletion, then deletes local consent metadata only when no stored custom voice references it; referenced consents fail closed with `voice_consent_in_use` so local state cannot keep dangling voice records |
| `POST /v1/audio/voices` | Implemented locally | Accepts the official no-query create shape before body parsing; accepts official-style multipart `name`, `consent`, and `audio_sample` plus JSON/base64 compatibility; requires an existing local `cons_*` id and enforces a local 20-voice cap |
| `GET /v1/audio/voices` | Local listing extension | Lists local custom voice metadata for local UI and SDK compatibility even though the current public OpenAPI path metadata only expands `createVoice`; only `after` and `limit` are accepted, unsupported query parameters such as `before` and `order` return 400 before listing, `limit` is validated from 1 through 100 with default 20, and scalar `after` may appear only once |
| `GET /v1/audio/voices/{voice_id}` | Local retrieval extension | Rejects unsupported query parameters before reading local state, then retrieves local custom voice metadata by id; custom voice ids can also be passed as `/v1/audio/speech` `voice` values for placeholder speech protocol tests |
| `DELETE /v1/audio/voices/{voice_id}` | Local cleanup extension | Rejects unsupported query parameters before deletion, then deletes local custom voice metadata and returns an OpenAI-style deleted object so long-running compatibility smoke tests can clean up bounded local state |
...
| `GET /v1/videos` | Implemented locally | Lists local video jobs with only the official `limit`, `after`, and `order` pagination parameters; `limit` is validated from 0 through 100 with default 20, `limit=0` returns an empty page with `has_more` when records exist, scalar `after` / `order` / `limit` may appear only once, `order` is validated against `asc` / `desc`, and unsupported query parameters such as `before` return 400 before listing |
