...
| Native hosted background durability full parity | Local background jobs are file-backed and can resume provider calls after `provider_pending`, and can resume local tool/context preparation from persisted `ready` step checkpoints. This is still a local retry rather than OpenAI's hosted job service, and jobs interrupted while a local preparation step is actively `running` fail closed to avoid re-running side-effecting local tools | Add a persisted worker queue with per-step leases, retry policies, backoff, and multi-process ownership so active steps can be retried only when the underlying tool is proven idempotent |
