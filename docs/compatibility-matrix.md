...
| `text.format.type=text` | omitted/default or Chat `response_format:{type:"text"}` | Direct after OpenAI request validation. Responses `text.format` and direct Chat `response_format` must be objects whose `type` is one of `text`, `json_object`, or `json_schema` before provider calls |
