...
| `logit_bias`, `modalities`, `audio`, `prediction`, `parallel_tool_calls`, `prompt_cache_key`, `prompt_cache_retention`, `safety_identifier`, legacy `functions` / `function_call` | same-name Chat fields | Provider-aware Chat-native passthrough; DeepSeek defaults to filtering these unsupported fields and records forwarded/filtered names in `metadata.compatibility.chat_native_fields` |
| `n` | same-name Chat field, or non-streaming local fan-out for unsupported providers | Provider-aware Chat-native passthrough. For DeepSeek-compatible direct Chat requests with native Chat field forwarding disabled, `n:1` is treated as the single-choice default and audited, while non-streaming `n>1` is emulated by making up to `CODEXCOMPAT_CHAT_N_EMULATION_MAX` upstream Chat calls, merging returned `choices`, aggregating `usage`, preserving local `store:true` messages, and recording `metadata.compatibility.chat_passthrough.n`. Streaming request-side fan-out is not implemented yet; streaming requests are normalized to the provider's single-choice behavior and audited with `reason:"streaming_fanout_not_implemented"` |
...
web-search adapter when native Chat fields are not forwarded, non-streaming
OpenAI Chat `n>1` is locally fanned out for providers that do not support
native multi-choice generation, `stream_options` are removed on non-streaming requests,
...
as `logit_bias`, `modalities`, `audio`, `moderation`, `prediction`,
`parallel_tool_calls`, `prompt_cache_key`, `prompt_cache_retention`,
`safety_identifier`, and legacy `functions` / `function_call` are filtered
...
| `GET /v1/organization/usage/completions` | Implemented locally | Aggregates local `/v1/responses`, `/v1/responses/compact`, `/v1/responses/input_tokens`, `/v1/chat/completions`, and `/v1/completions` usage into `organization.usage.completions.result` buckets with token, request, project/user/hashed-API-key/model/batch/service-tier fields; falls back to zero rows when no events match |
...
| `CODEXCOMPAT_CHAT_N_EMULATION_MAX` | `10` | Maximum local provider calls used to emulate unsupported non-streaming Direct Chat `n>1`; capped at 50 |
