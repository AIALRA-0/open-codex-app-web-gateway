...
| `GET /v1/organization/projects/{project_id}/certificates` | Implemented locally | Lists local project certificate records for active projects as `organization.project.certificate`, including project-scope `active` flags and no PEM content, with only the official certificate-list `limit`, `after`, and `order` query parameters plus OpenAI SDK `ConversationCursorPage` semantics; repeated scalar values return 400, `limit` is enforced from 1 through 100 with local default 20, official `order:"asc"|"desc"` is supported, unsupported query parameters such as `before` return 400 before listing, returns `last_id` without legacy `first_id`, and archived projects return `400 project_archived` |
