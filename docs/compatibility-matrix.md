...
| `POST /v1/fine_tuning/checkpoints/{fine_tuned_model_checkpoint}/permissions` | Implemented locally | Accepts the official `project_ids` string array and returns the created or existing local permissions as an OpenAI-style list; unknown JSON body fields are rejected, `project_ids` is required and must be an array of strings, and an empty array returns an empty OpenAI-style list without mutating local permissions |
