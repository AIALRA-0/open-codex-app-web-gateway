...
| `CODEXCOMPAT_FORWARD_CHAT_NATIVE_FIELDS` | auto: `false` for DeepSeek, `true` otherwise | Forwards provider-specific Chat request fields such as `logit_bias`, `modalities`, `audio`, `prediction`, `n`, `parallel_tool_calls`, `prompt_cache_key`, `prompt_cache_retention`, `safety_identifier`, `moderation`, `verbosity`, `web_search_options`, and legacy `functions/function_call`; DeepSeek defaults to filtering those OpenAI-only fields while continuing to forward DeepSeek-supported Chat fields such as `logprobs` / `top_logprobs`, and filtered requests are recorded in compatibility metadata |
