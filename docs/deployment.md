...
| `CODEXCOMPAT_FORWARD_CHAT_NATIVE_FIELDS` | auto: `false` for DeepSeek, `true` otherwise | Forwards provider-specific Chat request fields such as `logit_bias`, `modalities`, `audio`, `prediction`, `parallel_tool_calls`, `prompt_cache_key`, `prompt_cache_retention`, `safety_identifier`, `moderation`, `verbosity`, `web_search_options`, and legacy `functions/function_call`; DeepSeek defaults to filtering those OpenAI-only fields while continuing to forward DeepSeek-supported Chat fields such as `logprobs` / `top_logprobs`, and filtered requests are recorded in compatibility metadata. Direct Chat `n` is handled separately so unsupported multi-choice requests can be locally emulated instead of only filtered |
| `CODEXCOMPAT_CHAT_N_EMULATION_MAX` | `10` | Maximum local provider calls used to emulate unsupported Direct Chat `n>1` in non-streaming and streaming modes; values are bounded to 1-50 and capped requests are recorded in `metadata.compatibility.chat_passthrough.n.capped_to` |
| `CODEXCOMPAT_FORWARD_CHAT_CUSTOM_TOOLS` | auto: `false` for DeepSeek, `true` otherwise | Forwards OpenAI Chat custom tools to upstream providers that support non-function tools, including Responses `tools[type=custom]`, custom `tool_choice` selectors, and native `custom_tool_call` replay; DeepSeek defaults to filtering custom tools and invalid custom `tool_choice` values while preserving function tools and replaying custom tool context as text |
...
npm run bench:swe:score -- --prediction-report /srv/aialra/data/opencodexapp/eval/swebench/report.json --preflight-only --min-free-gb 120
